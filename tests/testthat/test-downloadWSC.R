...
# Ensure that WSC can be reached using fread for speed
...
datetime_string <- paste0(
  "start_date=",
  substr(start_dt, 1, 10),
  "%20",
  substr(start_dt, 12, 19),
  "&end_date=",
  substr(end_dt, 1, 10),
  "%20",
  substr(end_dt, 12, 19)
...
if (skip_test) {
  skip("WSC data cannot be fetched, skipping downloadWSC tests")
}
