...
predictMissing <- function(tsid, radius, predict_range, train_range = NULL, extra_params = NULL, imputed = TRUE, daily = FALSE, mice = FALSE, con = AquaCacheCon(silent = TRUE)) {
  ...
  if (daily) {
    ...
    if (imputed) {
      ...
      if (is.na(exist.values[exist.values$datetime == max(exist.values$datetime), "value"])) {
        ...
        goto_date <- DBI::dbGetQuery(con, paste0("SELECT min(date) FROM calculated_daily WHERE timeseries_id = ", tsid, " AND date > '", as.Date(end_predict), "' AND value IS NOT NULL;"))[1,1]
      ...
      if (is.na(exist.values[exist.values$datetime == min(exist.values$datetime), "value"])) {
        gofrom_date <- DBI::dbGetQuery(con, "SELECT max(date) FROM calculated_daily WHERE timeseries_id = ", tsid, " AND value IS NOT NULL AND date < '", as.Date(start_predict), "';")[1,1]
      ...
      if (is.na(exist.values[exist.values$datetime == max(exist.values$datetime), "value"])) {
        # Find the next non-NA value and add data to that point
        goto_date <- DBI::dbGetQuery(con, paste0("SELECT min(date) FROM calculated_daily WHERE timeseries_id = ", tsid, " AND date > '", as.Date(end_predict), "' AND value IS NOT NULL AND imputed = FALSE;"))[1,1]
      ...
      # Check to make sure the first value is not NA UNLESS the start_predict is prior to the entry in timeseries table
      if (is.na(exist.values[exist.values$datetime == min(exist.values$datetime), "value"])) {
        gofrom_date <- DBI::dbGetQuery(con, "SELECT max(date) FROM calculated_daily WHERE timeseries_id = ", tsid, " AND value IS NOT NULL AND imputed = FALSEAND date < '", as.Date(start_predict), "';")[1,1]
        if (gofrom_date < start_predict) {
          # Find the next non-NA value and add data to that point
  ...
  for (i in 1:length(selected)) {
    ...
    combined_predictDataList[[i]] <- combined_predictData
