...
message(
  "Working on Patch 6. Changes are being made within a transaction, so if something goes wrong, the database will be rolled back to its previous state (but you have a backup, right?)."
)
...
tryCatch(
  {
    ...
    message(
      "Creating new users so that postgres stops being used. ENTER YOUR DESIRED PASSWORD BELOW."
    )
    ...
    if (nrow(role_exists) == 0) {
      ...
      DBI::dbExecute(
        con,
        paste0("CREATE ROLE admin WITH LOGIN PASSWORD '", password, "';")
      )
    }
    ...
    DBI::dbExecute(
      ...
      paste0(
        ...
        DBI::dbQuoteIdentifier(con, current_db),
        " TO admin;"
      )
    )
  ...
  "
    ...
    DBI::dbExecute(
      con,
      "ALTER DEFAULT PRIVILEGES FOR ROLE postgres GRANT USAGE, CREATE ON SCHEMAS TO admin;"
    )
  ...
  "
    ...
    for (row in seq_len(nrow(sequences))) {
      ...
      DBI::dbExecute(
        ...
        paste0(
          ...
          DBI::dbQuoteIdentifier(con, schema),
          ".",
          DBI::dbQuoteIdentifier(con, sequence),
          " TO admin;"
        )
      )
    }
    ...
    DBI::dbExecute(
      con,
      "ALTER DEFAULT PRIVILEGES FOR ROLE postgres GRANT ALL PRIVILEGES ON TABLES TO admin;"
    )
    DBI::dbExecute(
      con,
      "ALTER DEFAULT PRIVILEGES FOR ROLE postgres GRANT ALL PRIVILEGES ON SEQUENCES TO admin;"
    )
    DBI::dbExecute(
      con,
      "ALTER DEFAULT PRIVILEGES FOR ROLE postgres GRANT EXECUTE ON FUNCTIONS TO admin;"
    )
    ...
    message(
      "Admin user setup completed for database ",
      DBI::dbGetQuery(con, "SELECT current_database();")[[1]]
    )
    ...
    DBI::dbExecute(
      con,
      "UPDATE information.version_info SET version = '6' WHERE item = 'Last patch number';"
    )
    DBI::dbExecute(
      con,
      paste0(
    ...
    message(
      "Patch 6 applied successfully: created new users. Now stop using postgres for everything, switch your 'postgres' entries in the .Renviron file to 'admin'!"
    )
  },
  error = function(e) {
    ...
    attr(con, "active_transaction") <<- FALSE
    stop(
      "Patch 6 failed and the DB has been rolled back to its earlier state. ",
      e$message
    )
  }
)
...
snow <- DBI::dbGetQuery(
  con,
  "SELECT datname FROM pg_database WHERE datname = 'snow';"
)
...
tryCatch(
  {
    message(
      "Now attempting to make 'admin' user get the same privileges on the snow database as the aquacache, if it exists.."
    )
    host <- DBI::dbGetQuery(
      con,
      "SELECT inet_server_addr() AS host_address, inet_server_port() AS port;"
    )
    snowCon <- snowConnect(
      username = "postgres",
    ...
    DBI::dbExecute(
      ...
      paste0(
        "GRANT CONNECT ON DATABASE ",
        DBI::dbQuoteIdentifier(snowCon, current_db),
        " TO admin;"
      )
    )
  ...
  "
    ...
    for (schema_name in schemas$schema_name) {
      DBI::dbExecute(
        ...
        paste0(
          "GRANT USAGE, CREATE ON SCHEMA ",
          DBI::dbQuoteIdentifier(snowCon, schema_name),
          " TO admin;"
        )
      )
    }
    ...
    for (row in seq_len(nrow(tables))) {
      ...
      DBI::dbExecute(
        ...
        paste0(
          ...
          ".",
          DBI::dbQuoteIdentifier(snowCon, table),
          " TO admin;"
        )
      )
    }
  ...
  "
    ...
    for (row in seq_len(nrow(sequences))) {
      ...
      DBI::dbExecute(
        ...
        paste0(
          ...
          DBI::dbQuoteIdentifier(snowCon, schema),
          ".",
          DBI::dbQuoteIdentifier(snowCon, sequence),
          " TO admin;"
        )
      )
    }
    ...
    message(
      "Admin user setup completed for database ",
      DBI::dbGetQuery(snowCon, "SELECT current_database();")[[1]]
    )

    # Check if 'snow_read' exists as a user. If not, create it
    snow_read_exists <- DBI::dbGetQuery(
      snowCon,
      "SELECT 1 FROM pg_roles WHERE rolname = 'snow_read';"
    )
    ...
    if (nrow(snow_read_exists) == 0) {
      ...
      message(
        "Created the 'snow_reader' account with default password of 'snow'"
      )
    }
    ...
    message(
      "Granting 'snow_reader' select privileges on all tables in 'snow'..."
    )
    tables <- DBI::dbGetQuery(
      snowCon,
      "
  ...
  error = function(e) {
    ...
    attr(snowCon, "active_transaction") <<- FALSE
    stop(
      "Snow database updates failed and were rolled back to an earlier state. ",
      e$message
    )
  }
)
