...
message("Working on Patch 4. Changes are being made within a transaction, so if something goes wrong, the database will be rolled back to its previous state (but you have a backup, right?).")
...
tryCatch({
  ...
  DBI::dbExecute(con, "ALTER TABLE measurements_discrete DROP COLUMN owner CASCADE") # Cascades to view table 'measurements_discrete_corrected'
  DBI::dbExecute(con, "ALTER TABLE measurements_discrete DROP COLUMN contributor CASCADE") # Cascades to view table 'measurements_discrete_corrected'
  ...
  DBI::dbExecute(con, "ALTER TABLE measurements_discrete ADD COLUMN analysis_datetime TIMESTAMP WITH TIME ZONE")
  # Drop the old unique key and recreate to include analysis_datetime
  DBI::dbExecute(con, "ALTER TABLE measurements_discrete DROP CONSTRAINT measurements_discrete_unique")
  DBI::dbExecute(con, "ALTER TABLE measurements_discrete ADD CONSTRAINT measurements_discrete_unique UNIQUE NULLS NOT DISTINCT (timeseries_id, datetime, sample_type, collection_method, sample_fraction, result_speciation, result_value_type, analysis_datetime)")
  ...
  DBI::dbExecute(con, "ALTER TABLE measurements_discrete SET SCHEMA measurements_discrete;")
  DBI::dbExecute(con, "ALTER TABLE sample_types SET SCHEMA measurements_discrete;")
  DBI::dbExecute(con, "ALTER TABLE result_value_types SET SCHEMA measurements_discrete;")
  DBI::dbExecute(con, "ALTER TABLE result_conditions SET SCHEMA measurements_discrete;")
  DBI::dbExecute(con, "ALTER TABLE collection_methods SET SCHEMA measurements_discrete;")
  DBI::dbExecute(con, "ALTER TABLE sample_fractions SET SCHEMA measurements_discrete;")
  DBI::dbExecute(con, "ALTER TABLE laboratories SET SCHEMA measurements_discrete;")
  DBI::dbExecute(con, "ALTER TABLE result_speciations SET SCHEMA measurements_discrete;")
  DBI::dbExecute(con, "ALTER TABLE analysis_protocols SET SCHEMA measurements_discrete;")
