...
message(
  "Working on Patch 13. Changes are being made within a transaction, so if something goes wrong, the database will be rolled back to its previous state (but you have a backup, right?)."
)

message(
  "This patch modifies the 'images' and 'documents' tables to add a 'tags' column (text array, indexed), fills in columns not already populated in the 'images' table and makes then NOT NULL, and modifies the 'locations' table to add columns to flag locations as having anthropogenic influence and as 'sentinel' locations for climate change analyses and reports."
)
...
tryCatch(
  {
    ...
    DBI::dbExecute(
      con,
      "ALTER TABLE locations ADD COLUMN anthropogenic_influence BOOLEAN DEFAULT FALSE;"
    )
    # Comment on the column
    DBI::dbExecute(
      con,
      "COMMENT ON COLUMN locations.anthropogenic_influence IS 'Flag to indicate if the location has anthropogenic influence';"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE locations ADD COLUMN sentinel_location BOOLEAN DEFAULT FALSE;"
    )
    # Comment on the column
    DBI::dbExecute(
      con,
      "COMMENT ON COLUMN locations.sentinel_location IS 'Flag to indicate if the location is a sentinel location for climate change analyses and reports';"
    )
    ...
    DBI::dbExecute(
      con,
      "ALTER TABLE images ADD COLUMN location_id INTEGER REFERENCES locations(location_id)"
    )
    ...
    DBI::dbExecute(
      con,
      "ALTER TABLE images RENAME COLUMN altitude_asl_m TO elevation_msl_m;"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE images RENAME COLUMN altitude_agl_m TO elevation_agl_m;"
    )
    # Add comments to both newly named columns
    DBI::dbExecute(
      con,
      "COMMENT ON COLUMN images.elevation_msl_m IS 'Elevation in meters above mean sea level';"
    )
    DBI::dbExecute(
      con,
      "COMMENT ON COLUMN images.elevation_agl_m IS 'Elevation in meters above ground level';"
    )

    # Drop trigger trigger_enforce_lat_lon_constraints and function enforce_lat_lon_constraints
    DBI::dbExecute(
      con,
      "DROP TRIGGER trigger_enforce_lat_lon_constraints ON images;"
    )
    ...
    for (i in unique(exists$img_meta_id)) {
      loc_id <- DBI::dbGetQuery(
        con,
        paste0(
          "SELECT location_id FROM images_index WHERE img_meta_id = ",
          i,
          ";"
        )
      )[1, 1]
      ...
      if (is.na(wsc)) {
        ...
        if (
          DBI::dbGetQuery(
            con,
            paste0(
              "SELECT source_fx FROM images_index WHERE img_meta_id = ",
              i,
              ";"
            )
          )[1, 1] %in%
            c("downloadWSCImages", "downloadNupointImages")
        ) {
          DBI::dbExecute(
            con,
            paste0(
    ...
    DBI::dbExecute(
      con,
      "ALTER TABLE images ALTER COLUMN latitude SET NOT NULL;"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE images ALTER COLUMN longitude SET NOT NULL;"
    )

    # Ensure that img_meta_id CAN be NULL
    DBI::dbExecute(
      con,
      "ALTER TABLE images ALTER COLUMN img_meta_id DROP NOT NULL;"
    )

    # Find the name of the unique key constraint and delete it
    key <- DBI::dbGetQuery(
      con,
      "SELECT constraint_name FROM information_schema.table_constraints WHERE table_name = 'images' AND constraint_type = 'UNIQUE'"
    )[1, 1]
    ...
    DBI::dbExecute(
      con,
      "DELETE FROM images WHERE octet_length(file) < 10 * 1024;"
    )
  ...
  "
    ...
    DBI::dbExecute(
      con,
      "COMMENT ON COLUMN images.file_hash IS 'MD5 hash of the image file, used to ensure that no two images are identical';"
    )
...
"
  ...
  "
    ...
    DBI::dbExecute(
      con,
      "COMMENT ON COLUMN documents.file_hash IS 'MD5 hash of the document file, used for ensuring that no two documents are identical.';"
    )
...
"
    ...
    DBI::dbExecute(
      con,
      "ALTER TABLE documents_spatial DROP CONSTRAINT documents_spatial_document_id_fkey;"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE documents_spatial DROP CONSTRAINT documents_spatial_geom_id_fkey"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE documents_spatial ADD CONSTRAINT documents_spatial_document_id_fkey FOREIGN KEY (document_id) REFERENCES documents(document_id) ON DELETE CASCADE ON UPDATE CASCADE;"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE documents_spatial ADD CONSTRAINT documents_spatial_geom_id_fkey FOREIGN KEY (geom_id) REFERENCES vectors(geom_id) ON DELETE CASCADE ON UPDATE CASCADE;"
    )
  ...
  "
    ...
    DBI::dbExecute(
      con,
      "UPDATE information.version_info SET version = '13' WHERE item = 'Last patch number';"
    )
    DBI::dbExecute(
      con,
      paste0(
  ...
  error = function(e) {
    ...
    attr(con, "active_transaction") <<- FALSE
    stop(
      "Patch 13 failed and the DB has been rolled back to its earlier state. ",
      e$message
    )
  }
)
