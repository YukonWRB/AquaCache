...
message(
  "Working on Patch 11. Changes are being made within a transaction, so if something goes wrong, the database will be rolled back to its previous state (but you have a backup, right?)."
)
...
"
    ...
    DBI::dbExecute(
      con,
      "ALTER TABLE measurements_calculated_daily SET SCHEMA continuous;"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE measurements_continuous SET SCHEMA continuous;"
    )
    ...
    DBI::dbExecute(
      con,
      "ALTER TABLE rating_curve_points SET SCHEMA continuous;"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE rating_curve_shifts SET SCHEMA continuous;"
    )
    ...
    DBI::dbExecute(
      con,
      "ALTER VIEW measurements_calculated_daily_corrected SET SCHEMA continuous;"
    )
    DBI::dbExecute(
      con,
      "ALTER VIEW measurements_continuous_corrected SET SCHEMA continuous;"
    )
    DBI::dbExecute(
      con,
      "ALTER VIEW measurements_hourly_corrected SET SCHEMA continuous;"
    )
    DBI::dbExecute(
      con,
      "ALTER FUNCTION apply_corrections SET SCHEMA continuous;"
    )
    DBI::dbExecute(
      con,
      "ALTER FUNCTION check_approvals_overlap SET SCHEMA continuous;"
    )
    DBI::dbExecute(
      con,
      "ALTER FUNCTION check_contributors_overlap SET SCHEMA continuous;"
    )
    DBI::dbExecute(
      con,
      "ALTER FUNCTION check_grades_overlap SET SCHEMA continuous;"
    )
    DBI::dbExecute(
      con,
      "ALTER FUNCTION check_owners_overlap SET SCHEMA continuous;"
    )
    DBI::dbExecute(
      con,
      "ALTER FUNCTION check_qualifiers_overlap SET SCHEMA continuous;"
    )
    DBI::dbExecute(
      con,
      "ALTER FUNCTION delete_old_forecasts SET SCHEMA continuous;"
    )
    DBI::dbExecute(
      con,
      "ALTER FUNCTION validate_corrections SET SCHEMA continuous;"
    )

    DBI::dbExecute(
      con,
      "ALTER TABLE owners_contributors RENAME TO organizations;"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE organizations RENAME COLUMN owner_contributor_id TO organization_id"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE continuous.owners RENAME COLUMN owner_contributor_id TO organization_id"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE continuous.contributors RENAME COLUMN owner_contributor_id TO organization_id"
    )
...
"
    ...
    DBI::dbExecute(
      con,
      "ALTER FUNCTION check_location_images SET SCHEMA files;"
    )
    DBI::dbExecute(
      con,
      "ALTER FUNCTION check_data_sharing_agreement SET SCHEMA files;"
    )
    DBI::dbExecute(
      con,
      "ALTER FUNCTION enforce_lat_lon_constraints SET SCHEMA files;"
    )
    DBI::dbExecute(
      con,
      "ALTER FUNCTION enforce_share_with_restriction SET SCHEMA files;"
    )
    DBI::dbExecute(
      con,
      "ALTER FUNCTION update_document_flags_after_delete SET SCHEMA files;"
    )
    DBI::dbExecute(
      con,
      "ALTER FUNCTION update_document_flags_after_insert SET SCHEMA files;"
    )
    ...
    DBI::dbExecute(
      con,
      "ALTER SCHEMA measurements_discrete RENAME TO discrete;"
    )
    DBI::dbExecute(
      con,
...
"
    ...
    DBI::dbExecute(
      con,
      "ALTER DATABASE aquacache SET search_path TO public, continuous, discrete, spatial, files, instruments, information;"
    )
    ...
    DBI::dbExecute(
      con,
      "GRANT USAGE ON SCHEMA public, continuous, discrete, spatial, files, instruments, information TO public_reader, discrete_editor, continuous_editor, yg;"
    )
  ...
  "
    ...
    DBI::dbExecute(
      con,
      paste0(
        "CREATE INDEX sub_locations_share_with_gin_idx ON sub_locations USING GIN (share_with);"
      )
    )
    ...
    DBI::dbExecute(
      con,
      paste0(
        "CREATE POLICY rls ON sub_locations
                              FOR ALL
                              USING (
...
"
    ...
    DBI::dbExecute(
      con,
      "ALTER TABLE locations_metadata_instruments OWNER TO admin;"
    )
...
"
    ...
    DBI::dbExecute(
      con,
      "ALTER TABLE locations_metadata_access ADD COLUMN sub_location_id INTEGER REFERENCES sub_locations(sub_location_id);"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE locations_metadata_infrastructure ADD COLUMN sub_location_id INTEGER REFERENCES sub_locations(sub_location_id);"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE locations_metadata_infrastructure_groundwater ADD COLUMN sub_location_id INTEGER REFERENCES sub_locations(sub_location_id);"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE locations_metadata_infrastructure_hydromet ADD COLUMN sub_location_id INTEGER REFERENCES sub_locations(sub_location_id);"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE locations_metadata_maintenance ADD COLUMN sub_location_id INTEGER REFERENCES sub_locations(sub_location_id);"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE locations_metadata_owners_operators ADD COLUMN sub_location_id INTEGER REFERENCES sub_locations(sub_location_id);"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE locations_metadata_xsections ADD COLUMN sub_location_id INTEGER REFERENCES sub_locations(sub_location_id);"
    )

    DBI::dbExecute(
      con,
      "ALTER TABLE locations_metadata_access DROP CONSTRAINT IF EXISTS locations_metadata_access_unique;"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE locations_metadata_infrastructure DROP CONSTRAINT IF EXISTS locations_metadata_infrastructure_unique;"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE locations_metadata_infrastructure_groundwater DROP CONSTRAINT IF EXISTS locations_metadata_infrastructure_groundwater_unique;"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE locations_metadata_infrastructure_hydromet DROP CONSTRAINT IF EXISTS locations_metadata_infrastructure_hydromet_unique;"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE locations_metadata_maintenance DROP CONSTRAINT IF EXISTS locations_metadata_maintenance_unique;"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE locations_metadata_owners_operators DROP CONSTRAINT IF EXISTS locations_metadata_owners_operators_unique;"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE locations_metadata_xsections DROP CONSTRAINT IF EXISTS locations_metadata_xsections_unique;"
    )

    DBI::dbExecute(
      con,
      "ALTER TABLE locations_metadata_access ADD CONSTRAINT locations_metadata_access_unique UNIQUE NULLS NOT DISTINCT (location_id, sub_location_id);"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE locations_metadata_infrastructure ADD CONSTRAINT locations_metadata_infrastructure_unique UNIQUE NULLS NOT DISTINCT (location_id, sub_location_id);"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE locations_metadata_infrastructure_groundwater ADD CONSTRAINT locations_metadata_infrastructure_groundwater_unique UNIQUE NULLS NOT DISTINCT (location_id, sub_location_id);"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE locations_metadata_infrastructure_hydromet ADD CONSTRAINT locations_metadata_infrastructure_hydromet_unique UNIQUE NULLS NOT DISTINCT (location_id, sub_location_id);"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE locations_metadata_maintenance ADD CONSTRAINT locations_metadata_maintenance_unique UNIQUE NULLS NOT DISTINCT (location_id, sub_location_id);"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE locations_metadata_owners_operators ADD CONSTRAINT locations_metadata_owners_operators_unique UNIQUE NULLS NOT DISTINCT (location_id, sub_location_id);"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE locations_metadata_xsections ADD CONSTRAINT locations_metadata_xsections_unique UNIQUE NULLS NOT DISTINCT (location_id, sub_location_id);"
    )
    ...
    DBI::dbExecute(
      con,
      "UPDATE locations SET owner = 2 WHERE location LIKE 'YOWN%';"
    )

    tmp <- DBI::dbGetQuery(
      con,
      "SELECT location_id, owner FROM locations WHERE owner IS NOT NULL"
    )

    for (i in unique(tmp$location_id)) {
      # Fetch the earliest datetime for this location from table 'timeseries'
      start_dt <- DBI::dbGetQuery(
        con,
        paste0(
          "SELECT MIN(start_datetime) FROM timeseries WHERE location_id = ",
          i
        )
      )[1, 1]
      if (is.na(start_dt)) {
      ...
      df <- data.frame(
        location_id = i,
        owner = tmp[tmp$location_id == i, "owner"],
        operator = tmp[tmp$location_id == i, "owner"],
...
"
    ...
    DBI::dbExecute(
      con,
      "COMMENT ON TABLE discrete.sample_series IS
    ...
    DBI::dbExecute(
      con,
      "CREATE INDEX idx_samples_location_id ON discrete.samples(location_id);"
    )
    DBI::dbExecute(
      con,
      "CREATE INDEX idx_samples_datetime ON discrete.samples(datetime);"
    )
    DBI::dbExecute(
      con,
      "CREATE INDEX idx_samples_media_id ON discrete.samples(media_id);"
    )
    ...
    DBI::dbExecute(
      con,
      paste0(
        "CREATE INDEX samples_share_with_gin_idx ON samples USING GIN (share_with);"
      )
    )
...
"
    ...
    DBI::dbExecute(
      con,
      "UPDATE sample_types SET sample_type = 'sample-field msr/obs - no lab results expected' WHERE sample_type = 'field msr/obs';"
    )
    DBI::dbExecute(
      con,
      "INSERT INTO sample_types (sample_type) VALUES ('unknown');"
    )
    ...
    DBI::dbExecute(
      con,
      "ALTER TABLE analysis_protocols RENAME TO protocols_methods;"
    )
    ...
    DBI::dbExecute(
      con,
      "CREATE INDEX idx_results_sample_id ON discrete.results(sample_id);"
    )
    DBI::dbExecute(
      con,
      "CREATE INDEX idx_results_parameter_id ON discrete.results(parameter_id);"
    )
    DBI::dbExecute(
      con,
      "CREATE INDEX idx_results_sample_parameter ON discrete.results(sample_id, parameter_id);"
    )
    DBI::dbExecute(
      con,
      "CREATE INDEX idx_results_result_condition ON discrete.results(result_condition);"
    )
    ...
    DBI::dbExecute(
      con,
      paste0(
        "CREATE INDEX results_share_with_gin_idx ON results USING GIN (share_with);"
      )
    )
  ...
  "
    ...
    DBI::dbExecute(
      con,
      "ALTER TABLE discrete.results ADD CONSTRAINT chk_result_condition CHECK (((result_condition IS NULL) OR (result IS NULL)))"
    )

    # if result_condition is 1 or 2, result_condition_value must be populated
    DBI::dbExecute(
      con,
      "ALTER TABLE discrete.results ADD CONSTRAINT chk_result_condition_value CHECK (((result_condition_value IS NULL) OR (result_condition = ANY (ARRAY[1, 2]))))"
    )
...
"
    ...
    sample_series <- DBI::dbGetQuery(
      con,
      "SELECT timeseries_id, location, location_id, sub_location_id, parameter_id, media_id, z, last_new_data, last_synchronize, source_fx, source_fx_args, note, active FROM timeseries WHERE category = 'discrete'"
    )

    # Find parameter_id for definite field parameters: SWE, snow depth
    field <- DBI::dbGetQuery(
      con,
      "SELECT parameter_id FROM parameters WHERE param_name IN ('snow depth', 'snow water equivalent', 'water level', 'flow')"
    )

    ## Move the data into sample_series, samples, and results
    message(
      "Moving discrete data to new table structure. This will take a while (I didn't code it efficiently at all, sorry)."
    )
    all_sample_series <- data.frame()
    for (i in unique(sample_series$location_id)) {
      if (
        !is.null(sample_series[sample_series$location_id == i, "source_fx"])
      ) {
        # Make entry in sample_series
        series <- data.frame(
          location_id = i,
          last_new_data = min(sample_series[
            sample_series$location_id == i,
            "last_new_data"
          ]),
          active = any(sample_series[sample_series$location_id == i, "active"]),
          source_fx = sample_series[
            sample_series$location_id == i,
            "source_fx"
          ][[1]],
          source_fx_args = if (
            all(is.na(sample_series[
              sample_series$location_id == i,
              "source_fx_args"
            ]))
          ) {
            ...
            paste(
              sample_series[sample_series$location_id == i, "source_fx_args"],
              collapse = " AND "
            )
          },
          note = if (
            all(is.na(sample_series[sample_series$location_id == i, "note"]))
          ) {
            NA
          } else {
            paste(
              sample_series[sample_series$location_id == i, "note"],
              collapse = " AND "
            )
          },
          default_owner = DBI::dbGetQuery(
            con,
            paste0(
              "SELECT DISTINCT owner FROM timeseries WHERE location_id = ",
              i,
              ";"
            )
          )[, 1],
          default_contributor = DBI::dbGetQuery(
            con,
            paste0(
              "SELECT DISTINCT owner FROM timeseries WHERE location_id = ",
              i,
    ...
    all_meas <- data.frame()
    count <- 1
    for (i in unique(measurements$location_id)) {
      # Create a series of entries to the 'sample' table for each sampling event
      message(
        "Processing location ",
        count,
        " of ",
        length(unique(measurements$location_id))
      )
      count <- count + 1
      sub <- measurements[measurements$location_id == i, ] # Isolate data specific to this location
      for (j in 1:length(unique(sub$datetime))) {
        # Make a record per sample datetime
        ...
        sample <- data.frame(
          ...
          datetime = datetime,
          target_datetime = unique(sub[
            sub$datetime == datetime,
            "target_datetime"
          ]),
          collection_method = unique(sub[
            sub$datetime == datetime,
            "collection_method"
          ]),
          ...
          owner = DBI::dbGetQuery(
            ...
            paste0(
              "SELECT DISTINCT owner FROM timeseries WHERE location_id = ",
              unique(sub[sub$datetime == datetime, "location_id"]),
              ";"
            )
          )[, 1],
          ...
          share_with = if (
            all(is.na(sub[sub$datetime == datetime, "share_with"]))
          ) {
            NA
          } else {
            unique(sub[sub$datetime == datetime, "share_with"])
          },
          ...
          note = if (all(is.na(sub[sub$datetime == datetime, "note"]))) {
            NA
          } else {
            paste(sub[sub$datetime == datetime, "note"], collapse = " AND ")
          }
        )
        DBI::dbAppendTable(con, "samples", sample)

        # Get the sample_id for this sample
        new_sample_id <- DBI::dbGetQuery(
          con,
          paste0(
        ...
        new_meas <- data.frame(
          ...
          result_type = ifelse(
            sub[sub$datetime == datetime, "parameter_id"] %in%
              field$parameter_id,
            1,
            3
          ),
    ...
    DBI::dbExecute(
      con,
      "ALTER TABLE timeseries RENAME COLUMN owner TO default_owner;"
    )
...
WHERE conrelid = 'timeseries'::regclass
  ...
  "
    ...
    DBI::dbExecute(
      con,
      "UPDATE internal_status SET event = 'last_sync_continuous' WHERE event = 'last_sync'"
    )
    DBI::dbExecute(
      con,
      "INSERT INTO internal_status (event) VALUES ('last_sync_discrete')"
    )
    ...
    try({
      ...
      DBI::dbExecute(
        con,
        "ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public REVOKE ALL ON TABLES FROM ac_editor;"
      )
      DBI::dbExecute(
        con,
        "ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA continuous REVOKE ALL ON TABLES FROM ac_editor;"
      )
      DBI::dbExecute(
        con,
        "ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA discrete REVOKE ALL ON TABLES FROM ac_editor;"
      )
      DBI::dbExecute(
        con,
        "ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA spatial REVOKE ALL ON TABLES FROM ac_editor;"
      )
      DBI::dbExecute(
        con,
        "ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA files REVOKE ALL ON TABLES FROM ac_editor;"
      )
      DBI::dbExecute(
        con,
        "ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA instruments REVOKE ALL ON TABLES FROM ac_editor;"
      )
      DBI::dbExecute(
        con,
        "ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA information REVOKE ALL ON TABLES FROM ac_editor;"
      )

      DBI::dbExecute(
        con,
        "ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public REVOKE ALL ON SEQUENCES FROM ac_editor;"
      )
      DBI::dbExecute(
        con,
        "ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA continuous REVOKE ALL ON SEQUENCES FROM ac_editor;"
      )
      DBI::dbExecute(
        con,
        "ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA discrete REVOKE ALL ON SEQUENCES FROM ac_editor;"
      )
      DBI::dbExecute(
        con,
        "ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA spatial REVOKE ALL ON SEQUENCES FROM ac_editor;"
      )
      DBI::dbExecute(
        con,
        "ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA files REVOKE ALL ON SEQUENCES FROM ac_editor;"
      )
      DBI::dbExecute(
        con,
        "ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA instruments REVOKE ALL ON SEQUENCES FROM ac_editor;"
      )
      DBI::dbExecute(
        con,
        "ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA information REVOKE ALL ON SEQUENCES FROM ac_editor;"
      )
...
"
    ...
    extra_tbls <- c(
      ...
      "locations_metadata_infrastructure_groundwater",
      "locations_metadata_infrastructure_hydromet",
    ...
    files_tbls <- DBI::dbGetQuery(
      con,
      "SELECT table_name FROM information_schema.tables WHERE table_schema = 'files'"
    )[, 1]
    # Remove 'images_index' and 'document_types'
    files_tbls <- files_tbls[
      files_tbls != "images_index" & files_tbls != "document_types"
    ]
    instruments_tbls <- DBI::dbGetQuery(
      con,
      "SELECT table_name FROM information_schema.tables WHERE table_schema = 'instruments'"
    )[, 1]
    spatial_tbls <- DBI::dbGetQuery(
      con,
      "SELECT table_name FROM information_schema.tables WHERE table_schema = 'spatial'"
    )[, 1]

    # Get all tables in schema 'discrete'
    discrete_schema_tbls <- DBI::dbGetQuery(
      con,
      "SELECT table_name FROM information_schema.tables WHERE table_schema = 'discrete'"
    )[, 1]
    # Remove 'sample_series'
    discrete_schema_tbls <- discrete_schema_tbls[
      discrete_schema_tbls != "sample_series"
    ]
    discrete_editor_tbls <- c(
      extra_tbls,
      discrete_schema_tbls,
      instruments_tbls,
      files_tbls,
      spatial_tbls
    )
    ...
    continuous_schema_tbls <- DBI::dbGetQuery(
      con,
      "SELECT table_name FROM information_schema.tables WHERE table_schema = 'continuous'"
    )[, 1]
    # Remove 'fetch_settings'
    continuous_schema_tbls <- continuous_schema_tbls[
      continuous_schema_tbls != "fetch_settings"
    ]
    continuous_editor_tbls <- c(
      extra_tbls,
      continuous_schema_tbls,
      instruments_tbls,
      files_tbls,
      spatial_tbls
    )
    ...
    discrete_schema_seqs <- DBI::dbGetQuery(
      con,
      "
...
"
    ...
    continuous_schema_seqs <- DBI::dbGetQuery(
      con,
      "
  SELECT sequence_name
  FROM information_schema.sequences
...
"
    ...
    public_schema_seqs <- DBI::dbGetQuery(
      con,
      "
  SELECT sequence_name
  FROM information_schema.sequences
...
"
    ...
    instrument_schema_seqs <- DBI::dbGetQuery(
      con,
      "
  SELECT sequence_name
  FROM information_schema.sequences
...
"
    ...
    spatial_schema_seqs <- DBI::dbGetQuery(
      con,
      "
  SELECT sequence_name
  FROM information_schema.sequences
...
"
    ...
    files_schema_seqs <- DBI::dbGetQuery(
      con,
      "
  SELECT sequence_name
  FROM information_schema.sequences
...
"
    ...
    sequences_cont <- c(
      continuous_schema_seqs,
      public_schema_seqs,
      instrument_schema_seqs,
      files_schema_seqs
    )
    sequences_disc <- c(
      discrete_schema_seqs,
      public_schema_seqs,
      instrument_schema_seqs,
      files_schema_seqs
    )
    ...
    try({
      ...
      DBI::dbExecute(
        con,
        "UPDATE timeseries SET share_with = '{yg_reader}' WHERE share_with = '{yg}';"
      )
      DBI::dbExecute(
        con,
        "UPDATE locations SET share_with = '{yg_reader}' WHERE share_with = '{yg}';"
      )
    })
...
"
  ...
  "
    ...
    DBI::dbExecute(
      con,
      "UPDATE information.version_info SET version = '11' WHERE item = 'Last patch number';"
    )
    DBI::dbExecute(
      con,
      paste0(
    ...
    message(
      "Success! Patch 11 applied successfully: discrete data tables have been modified for better organization and location + sub-location metadata storage was improved."
    )
  },
  error = function(e) {
    ...
    attr(con, "active_transaction") <<- FALSE
    stop(
      "Patch 11 failed and the DB has been rolled back to its earlier state. ",
      e$message
    )
  }
)
