...
message(
  "Working on Patch 20. Changes are being made within a transaction, so if something goes wrong, the database will be rolled back to its previous state (but you have a backup, right?)."
)

message(
  "This patch modifies the column 'period_type' of table 'timeseries'. This column gets renamed aggregation_type_id, and it will now refer to a table."
)
...
tryCatch(
  {
    ...
    DBI::dbExecute(
      con,
      "ALTER TABLE timeseries DROP CONSTRAINT IF EXISTS timeseries_period_type_check;"
    )
    # Rename column period_type to aggregation_type_id
    DBI::dbExecute(
      con,
      "ALTER TABLE timeseries RENAME COLUMN period_type TO aggregation_type_id;"
    )
    ...
    tbl <- data.frame(
      ...
      description = c(
        "Instantaneous value, no period assigned.",
      ...
      description_fr = c(
        "Valeur instantanée, pas de période assignée.",
    ...
    for (i in 1:nrow(aggregation_types)) {
      DBI::dbExecute(
        ...
        paste0(
          ...
          aggregation_types$aggregation_type_id[i],
          " WHERE aggregation_type_id = '",
          aggregation_types$aggregation_type[i],
          "';"
        )
      )
    }
    ...
    DBI::dbExecute(
      con,
      "ALTER TABLE timeseries ALTER COLUMN aggregation_type_id TYPE INTEGER USING aggregation_type_id::integer;"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE timeseries ALTER COLUMN aggregation_type_id SET NOT NULL;"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE timeseries ADD CONSTRAINT fk_aggregation_type FOREIGN KEY (aggregation_type_id) REFERENCES aggregation_types(aggregation_type_id);"
    )

    # Rename the 'min' and 'max' of table aggregation_types to 'minimum' and 'maximum'
    DBI::dbExecute(
      con,
      "UPDATE aggregation_types SET aggregation_type = 'minimum' WHERE aggregation_type = 'min';"
    )
    DBI::dbExecute(
      con,
      "UPDATE aggregation_types SET aggregation_type = 'maximum' WHERE aggregation_type = 'max';"
    )
    ...
    DBI::dbExecute(
      con,
      "UPDATE timeseries SET record_rate = '1 hour' WHERE record_rate < '1 day' AND source_fx != 'downloadWSC';"
    )
    DBI::dbExecute(
      con,
      "UPDATE timeseries SET record_rate = '5 minutes' WHERE record_rate < '1 day' AND source_fx = 'downloadWSC';"
    )
    # Now we can convert the column to INTERVAL
    DBI::dbExecute(
      con,
      "ALTER TABLE timeseries ALTER COLUMN record_rate TYPE INTERVAL USING record_rate::interval;"
    )
  ...
  '
    ...
    DBI::dbExecute(
      con,
      "COMMENT ON COLUMN timeseries.active IS 'Defines if the timeseries should or should not be added to or back-corrected by various AquaCache package functions.'"
    )
    DBI::dbExecute(
      con,
      "COMMENT ON COLUMN timeseries.record_rate IS 'The general recording interval for the timeseries, only used to differentiate between sub-daily, daily, weekly, or monthly recording. A more refined measure of the periodicity of the data is recorded in the measurements_continuous table.'"
    )
    DBI::dbExecute(
      con,
      "COMMENT ON COLUMN timeseries.source_fx IS 'The function used to download and pre-process data for addition to a timeseries. Must be a function present in the R package AquaCache.'"
    )
    DBI::dbExecute(
      con,
      "COMMENT ON COLUMN timeseries.source_fx_args IS 'Arguments to pass to the source_fx in a JSON format. Used for example to specify the url from which to fetch data or the location and parameter.'"
    )
    DBI::dbExecute(
      con,
      "COMMENT ON COLUMN timeseries.timeseries_id IS 'Autoincrements each time a timeseries is added. NOTE that timeseries should only be added using the R function addACTimeseries from the package AquaCache to ensure correctness of the data.'"
    )
    DBI::dbExecute(
      con,
      "COMMENT ON COLUMN timeseries.aggregation_type_id IS 'Descriptor for the type of aggregation applicable to this timeseries. Must match an entry to table aggregation_types, for example one of instantaneous, sum, mean, median, min, max, or (min+max)/2.'"
    )

    # Update the version_info table
    DBI::dbExecute(
      con,
      "UPDATE information.version_info SET version = '20' WHERE item = 'Last patch number';"
    )
    DBI::dbExecute(
      con,
      paste0(
  ...
  error = function(e) {
    ...
    attr(con, "active_transaction") <<- FALSE
    stop(
      "Patch 20 failed and the DB has been rolled back to its earlier state. ",
      e$message
    )
  }
)
