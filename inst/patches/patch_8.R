...
message(
  "Working on Patch 8. Changes are being made within a transaction, so if something goes wrong, the database will be rolled back to its previous state (but you have a backup, right?)."
)
...
"
  ...
  "
    ...
    DBI::dbExecute(
      con,
      "ALTER DEFAULT PRIVILEGES FOR ROLE postgres GRANT TRIGGER ON TABLES TO admin;"
    )

    # Update the version_info table
    DBI::dbExecute(
      con,
      "UPDATE information.version_info SET version = '8' WHERE item = 'Last patch number';"
    )
    DBI::dbExecute(
      con,
      paste0(
    ...
    message(
      "Patch 8 applied successfully: admin now has access to all views, functions, and triggers and defaults to have access to all in future."
    )
  },
  error = function(e) {
    ...
    attr(con, "active_transaction") <<- FALSE
    stop(
      "Patch 8 failed and the DB has been rolled back to its earlier state. ",
      e$message
    )
  }
)
    ...
    for (row in seq_len(nrow(views))) {
      ...
      DBI::dbExecute(
        ...
        paste0(
          ...
          DBI::dbQuoteIdentifier(snowCon, schema),
          ".",
          DBI::dbQuoteIdentifier(snowCon, view),
          " TO admin;"
        )
      )
    }
