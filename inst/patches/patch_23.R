...
message(
  "Working on Patch 23. Changes are being made within a transaction, so if something goes wrong, the database will be rolled back to its previous state (but you have a backup, right?)."
)
...
message(
  "Would you like to populate the boreholes schema with data from the Access and SQL Server databases? \n 1 = Yes \n 2 = No"
)
...
if (choice == "1") {
  message(
    "The boreholes schema will be populated with data from the Access and SQL Server databases."
  )
}
...
if (choice == "1") {
  ...
  message(
    "Is 'X:/YWWR/App/Database27.mdb' the correct path to the Access database? Enter 1 for yes or type in the correct path."
  )
  ...
  message(
    "Using the SQL Server database 'YWWR' on server 'sql-apps2-prd'. Ensure you have access to this database and the necessary permissions to create tables and schemas."
  )
...
tryCatch(
  {
    ...
    DBI::dbExecute(
      con,
      "create trigger trg_user_audit before update on boreholes.drillers for each row execute function user_modified()"
    )
    DBI::dbExecute(
      con,
      "create trigger update_modify_time before update on boreholes.drillers for each row execute function update_modified()"
    )
    ...
    DBI::dbExecute(
      con,
      "create trigger trg_user_audit before update on boreholes.boreholes for each row execute function user_modified()"
    )
    DBI::dbExecute(
      con,
      "create trigger update_modify_time before update on boreholes.boreholes for each row execute function update_modified()"
    )
    # Create RLS policy
    DBI::dbExecute(
      con,
    ...
    DBI::dbExecute(
      con,
      paste0(
        "CREATE POLICY rls ON  boreholes.boreholes
                              FOR ALL
                              USING (
  ...
  "
    ...
    DBI::dbExecute(
      con,
      "CREATE INDEX IF NOT EXISTS idx_borehole_name ON boreholes.boreholes(borehole_name);"
    )
    # Create an index on latitude and longitude for faster spatial queries
    DBI::dbExecute(
      con,
      "CREATE INDEX IF NOT EXISTS idx_borehole_lat_lon ON boreholes.boreholes(latitude, longitude);"
    )
    ...
    DBI::dbExecute(
      con,
      "create trigger trg_user_audit before update on boreholes.permafrost for each row execute function user_modified()"
    )
    DBI::dbExecute(
      con,
      "create trigger update_modify_time before update on boreholes.permafrost for each row execute function update_modified()"
    )
    ...
    DBI::dbExecute(
      con,
      "create trigger trg_user_audit before update on boreholes.geology for each row execute function user_modified()"
    )
    DBI::dbExecute(
      con,
      "create trigger update_modify_time before update on boreholes.geology for each row execute function update_modified()"
    )
    ...
    DBI::dbExecute(
      con,
      "create trigger trg_user_audit before update on boreholes.casing_materials for each row execute function user_modified()"
    )
    DBI::dbExecute(
      con,
      "create trigger update_modify_time before update on boreholes.casing_materials for each row execute function update_modified()"
    )
    ...
    DBI::dbExecute(
      con,
      "create trigger trg_user_audit before update on boreholes.wells for each row execute function user_modified()"
    )
    DBI::dbExecute(
      con,
      "create trigger update_modify_time before update on boreholes.wells for each row execute function update_modified()"
    )
    ...
    DBI::dbExecute(
      con,
      paste0(
        "CREATE POLICY rls ON  boreholes.wells
                              FOR ALL
                              USING (
                                ('public_reader' = ANY(share_with))
                                OR EXISTS (
  ...
  "
    ...
    new_types <- data.frame(
      ...
      document_type_fr = c(
        ...
        "profil géologique",
        "enregistrement de puits d''eau"
      )
    )

    # The document types might exist already, so only insert if they don't
    for (i in 1:nrow(new_types)) {
      type_exists <- DBI::dbGetQuery(
        con,
        paste0(
          "SELECT document_type_id FROM document_types WHERE document_type_en = '",
          new_types$document_type_en[i],
          "';"
        )
      )[1, 1]
      if (is.na(type_exists)) {
        DBI::dbExecute(
          con,
          paste0(
            "INSERT INTO document_types (document_type_en, document_type_fr) VALUES ('",
            new_types$document_type_en[i],
            "', '",
            new_types$document_type_fr[i],
            "');"
          )
        )
      }
    ...
    tbls <- DBI::dbGetQuery(
      con,
      "SELECT DISTINCT table_name FROM information_schema.columns WHERE column_name = 'share_with';"
    )
    for (i in 1:nrow(tbls)) {
      tbl <- tbls$table_name[i]
      # Check if the table already has a column called 'private_expiry'
      col_exists <- DBI::dbGetQuery(
        con,
        paste0(
          "SELECT column_name FROM information_schema.columns WHERE table_name = '",
          tbl,
          "' AND column_name = 'private_expiry';"
        )
      )
      if (nrow(col_exists) == 0) {
        ...
        DBI::dbExecute(
          con,
          paste0("ALTER TABLE ", tbl, " ADD COLUMN private_expiry DATE;")
        )
      }
    }
    ...
    tbls <- DBI::dbGetQuery(
      con,
      "SELECT table_schema, table_name FROM information_schema.tables WHERE table_type = 'BASE TABLE' AND table_schema NOT IN ('pg_catalog', 'information_schema', 'boreholes');"
    )
    ...
    for (i in 1:nrow(tbls)) {
      ...
      col_exists <- DBI::dbGetQuery(
        ...
        paste0(
          ...
          "' AND table_name = '",
          tbl,
          "' AND column_name IN ('created_by', 'modified_by', 'created', 'modified');"
        )
      )
      ...
      if (
        !all(
          c("created_by", "modified_by", "created", "modified") %in%
        ...
        DBI::dbExecute(
          con,
          paste0(
            "DROP TRIGGER IF EXISTS trg_user_audit ON ",
            schema,
            ".",
            tbl,
            ";"
        ...
        DBI::dbExecute(
          con,
          paste0(
            "DROP TRIGGER IF EXISTS update_modify_time ON ",
            schema,
            ".",
            tbl,
            ";"
        ...
        DBI::dbExecute(
          con,
          paste0(
            "CREATE TRIGGER trg_user_audit BEFORE UPDATE ON ",
            schema,
            ".",
            tbl,
        ...
        DBI::dbExecute(
          con,
          paste0(
            "CREATE TRIGGER update_modify_time BEFORE UPDATE ON ",
            schema,
            ".",
            tbl,
    ...
    if (access && sql_server) {
      ...
      boreholes_acc <- DBI::dbGetQuery(
        acc,
        "SELECT BoreholeID AS _import_borehole_id_Access, Permafrost, PFDepthFrm, PFDepthTo, Depth, DrillerCode, DrillerNames FROM 1_BOREHOLE;"
      )
      boreholes_sql <- DBI::dbGetQuery(
        sql,
        "SELECT BoreholeId AS _import_borehole_id_SQL, WellName, UTMZone, Easting, Northing, LocationSource, Purpose, DepthToBedrock, DrillYear, DrillMonth, DrillDay, WellDepth, StaticWaterLevel, EstimatedYield, TopOfScreen, BottomOfScreen, WellHeadStickUp FROM WellRecords;"
      )
      boreholes <- merge(
        boreholes_sql,
        boreholes_acc,
        by.y = "_import_borehole_id_Access",
        by.x = "_import_borehole_id_SQL",
        all.x = TRUE
      ) # We are purposely dropping any boreholes that are only in Access as these don't have a water well associated and are 'orphan' records
      ...
      rlang::check_installed(
        "oce",
        "for converting UTM coordinates to latitude and longitude"
      )
      for (i in 1:nrow(boreholes)) {
        res <- oce::utm2lonlat(
          boreholes$Easting[i],
          boreholes$Northing[i],
          zone = boreholes$UTMZone[i],
          hemisphere = "N"
        )
      ...
      boreholes$completion_date <- as.Date(paste(
        boreholes$DrillYear,
        boreholes$DrillMonth,
        boreholes$DrillDay,
        sep = "-"
      ))
      # Make any date before 1900 NA
      boreholes$completion_Date[
        boreholes$completion_date < as.Date("1900-01-01")
      ] <- NA

      # Deal with elevations. We'll extract elevation from a web service as they're all shit
      rlang::check_installed(
        "elevatr",
        "for fetching elevations from a web service"
      )
      ...
      for (i in 1:nrow(boreholes)) {
        ...
        elev_data <- elevatr::get_elev_point(
          locations = sf_pts,
          src = "aws",
          z = 14
        )
      ...
      new_drillers <- merge(
        new_drillers,
        drillers[, c("Driller_code", "Drilling Company Names")],
        by.x = 'name',
        by.y = 'Drilling Company Names'
      )
      boreholes$drilled_by <- NA
      for (i in 1:nrow(boreholes)) {
        new_id <- NA
        new_id <- new_drillers[
          new_drillers$Driller_code == boreholes$DrillerCode[i],
          "driller_id"
        ]
        if (is.na(new_id[1])) {
          # Also check for a match on column 'DrillerNames'
          new_id <- new_drillers[
            new_drillers$Driller_code == "Ukwn",
            "driller_id"
          ]
        }
        if (!is.na(new_id[1])) {
      ...
      no_coords <- boreholes_import[
        is.na(boreholes_import$latitude) | is.na(boreholes_import$longitude),
      ]

      # Drop the no_coords from the main boreholes_import data frame
      boreholes_import <- boreholes_import[
        !is.na(boreholes_import$latitude) & !is.na(boreholes_import$longitude),
      ]
      ...
      DBI::dbExecute(
        con,
        "CREATE INDEX IF NOT EXISTS idx_borehole_no_coords_name ON boreholes.boreholes_no_coords(borehole_name);"
      )
      # Create an index on latitude and longitude for faster spatial queries
      DBI::dbExecute(
        con,
        "CREATE INDEX IF NOT EXISTS idx_borehole_no_coords_lat_lon ON boreholes.boreholes_no_coords(latitude, longitude);"
      )
      ...
      DBI::dbExecute(
        ...
        "
                 CREATE TABLE IF NOT EXISTS boreholes.boreholes_no_coords_documents (
                  borehole_id INTEGER NOT NULL REFERENCES boreholes_no_coords.(borehole_id) ON DELETE CASCADE ON UPDATE CASCADE,
                  document_id INTEGER NOT NULL REFERENCES files.documents(document_id) ON DELETE CASCADE ON UPDATE CASCADE,
                  PRIMARY KEY (borehole_id, document_id)
    );"
      )
      ...
      message(
        "Imported boreholes with no coordinates into the boreholes.boreholes_no_coords table."
      )
      ...
      new_boreholes <- DBI::dbGetQuery(
        con,
        "SELECT borehole_id, import_borehole_id, borehole_name FROM boreholes.boreholes;"
      )
      ...
      for (i in 1:nrow(new_boreholes)) {
        ftp <- DBI::dbGetQuery(
          sql,
          paste0(
            "SELECT WellLog FROM WellRecords WHERE BoreholeId = ",
            new_boreholes$import_borehole_id[i],
            ";"
          )
        )[1, 1]
        additional_ftp <- DBI::dbGetQuery(
          sql,
          paste0(
            "SELECT DocumentLinks FROM WellRecords WHERE BoreholeId = ",
            new_boreholes$import_borehole_id[i],
            ";"
          )
        )[1, 1]
        if (!is.na(ftp)) {
          ...
          tryCatch(
            ...
            error = function(e) {
              warning(
                "Failed to download document for borehole ",
                new_boreholes$import_borehole_id[i],
                ": ",
                e$message
              )
          ...
          if (file.exists(path)) {
            ...
            tryCatch(
              {
                ...
                exists <- DBI::dbGetQuery(
                  con,
                  paste0(
                    "SELECT COUNT(*) FROM files.documents WHERE name = '",
                    name,
                    "';"
                  )
                )[1, 1]
                    ...
                    if (exists > 0) {
                      ...
                      if (exists > 0) {
                        ...
                        if (exists > 0) {
                          ...
                          if (exists > 0) {
                            stop(
                              "Too many documents with the same name exist. Please rename the document manually and try again."
                            )
                          }
                        }
                      }
                ...
                res <- insertACDocument(
                  path = path,
                  name = name,
                  type = "water well record",
                  description = "Water well or borehole drilling record.",
                  tags = c("water well", "well", "borehole"),
                  con = con
                )
            ...
            if (!is.na(new_doc)) {
              ...
              check <- DBI::dbGetQuery(
                con,
                paste0(
                  "SELECT COUNT(*) FROM boreholes.boreholes_documents WHERE borehole_id = ",
                  new_boreholes$borehole_id[i],
                  " AND document_id = ",
                  new_doc,
                  ";"
                )
              ...
              if (check == 0) {
                DBI::dbExecute(
                  con,
                  paste0(
                    "INSERT INTO boreholes.boreholes_documents (borehole_id, document_id) VALUES (",
                    new_boreholes$borehole_id[i],
                    ",",
                    new_doc,
                    ");"
              ...
              for (j in 1:length(files)) {
                ...
                if (tolower(name) != "report") {
                  ...
                  if (additional_increment > 1) {
                    ...
                    names[j] <- paste0(
                      "Additional document for borehole ",
                      borehole_name
                    )
                  }
                }
      ...
      new_boreholes_no_coords <- DBI::dbGetQuery(
        con,
        "SELECT borehole_id, import_borehole_id, borehole_name FROM boreholes.boreholes_no_coords;"
      )
      for (i in 1:nrow(new_boreholes_no_coords)) {
        ftp <- DBI::dbGetQuery(
          ...
          paste0(
            "SELECT WellLog FROM WellRecords WHERE BoreholeId = ",
            new_boreholes_no_coords$import_borehole_id[i],
            ";"
          )
        )[1, 1]
        if (!is.na(ftp)) {
          ...
          if (file.exists(path)) {
            ...
            if (!is.na(new_doc)) {
              ...
              check <- DBI::dbGetQuery(
                con,
                paste0(
                  "SELECT COUNT(*) FROM boreholes.boreholes_no_coords_documents WHERE borehole_id = ",
                  new_boreholes_no_coords$borehole_id[i],
                  " AND document_id = ",
                  new_doc,
                  ";"
                )
              ...
              if (check == 0) {
                DBI::dbExecute(
                  con,
                  paste0(
                    "INSERT INTO boreholes.boreholes_no_coords_documents (borehole_id, document_id) VALUES (",
                    new_boreholes_no_coords$borehole_id[i],
                    ",",
                    new_doc,
                    ");"
      ...
      boreholes_pf <- boreholes[
        boreholes$Permafrost &
          !is.na(boreholes$Permafrost) &
          !is.na(boreholes$latitude),
      ]
      for (i in 1:nrow(boreholes_pf)) {
        ...
        new_borehole_id <- DBI::dbGetQuery(
          con,
          paste0(
            "SELECT borehole_id FROM boreholes.boreholes WHERE import_borehole_id = '",
            boreholes_pf$`_import_borehole_id_SQL`[i],
            "';"
          )
        )[1, 1]
        if (!is.na(new_borehole_id)) {
          ...
          DBI::dbExecute(
            con,
            paste0(
              "INSERT INTO boreholes.permafrost (borehole_id, depth_from_m, depth_to_m) VALUES (",
              new_borehole_id,
              ", ",
              if (!is.na(boreholes_pf$PFDepthFrm[i])) {
                boreholes_pf$PFDepthFrm[i]
              } else {
                "NULL"
              },
              ", ",
              if (!is.na(boreholes_pf$PFDepthTo[i])) {
                boreholes_pf$PFDepthTo[i]
              } else {
                "NULL"
              },
              ");"
          ...
          message(
            "No matching borehole found for permafrost record with import_borehole_id ",
            boreholes_pf$`_import_borehole_id_SQL`[i]
          )
        }
      }
      ...
      casing_materials <- DBI::dbGetQuery(
        con,
        "SELECT casing_material_id, material_name FROM boreholes.casing_materials;"
      )
      ...
      for (i in 1:nrow(boreholes)) {
        ...
        new_borehole_id <- DBI::dbGetQuery(
          ...
          paste0(
            "SELECT borehole_id FROM boreholes.boreholes WHERE import_borehole_id = '",
            boreholes$`_import_borehole_id_SQL`[i],
            "';"
          )
        )[1, 1] # there could be no id if the borehole had no lat/long
        if (!is.na(new_borehole_id)) {
          # If possible find casing material from Access table 2_WELL, matching on boreholes$`_import_borehole_id_SQL`
          casing_material <- retry(DBI::dbGetQuery(
            acc,
            paste0(
              "SELECT CasingMatl FROM 2_WELL WHERE BoreholeID = ",
              boreholes$`_import_borehole_id_SQL`[i],
              ";"
            )
          )[1, 1])
          # Map the casing_material to the new casing_materials table's casing_material_id
          casing_material_id <- casing_materials[
            tolower(casing_materials$material_name) == tolower(casing_material),
            "casing_material_id"
          ]
          casing_diameter_mm <- retry(DBI::dbGetQuery(
            acc,
            paste0(
              "SELECT CasingDia FROM 2_WELL WHERE BoreholeID = ",
              boreholes$`_import_borehole_id_SQL`[i],
              ";"
            )
          )[1, 1])
          casing_diameter_in <- DBI::dbGetQuery(
            sql,
            paste0(
              "SELECT CasingOutsideDiameter FROM WellRecords WHERE BoreholeID = ",
              boreholes$`_import_borehole_id_SQL`[i],
              ";"
            )
          )[1, 1]
          casing_depth_to <- retry(DBI::dbGetQuery(
            acc,
            paste0(
              "SELECT C_DepthTo FROM 2_WELL WHERE BoreholeID = ",
              boreholes$`_import_borehole_id_SQL`[i],
              ";"
            )
          )[1, 1])
          stick_up_height <- DBI::dbGetQuery(
            sql,
            paste0(
              "SELECT WellHeadStickUp FROM WellRecords WHERE BoreholeID = ",
              boreholes$`_import_borehole_id_SQL`[i],
              ";"
            )
          )[1, 1]
          casing_comment <- retry(DBI::dbGetQuery(
            acc,
            paste0(
              "SELECT Case_Comm FROM 2_WELL WHERE BoreholeID = ",
              boreholes$`_import_borehole_id_SQL`[i],
              ";"
            )
          )[1, 1])
          screen_top <- DBI::dbGetQuery(
            sql,
            paste0(
              "SELECT TopOfScreen FROM WellRecords WHERE BoreholeID = ",
              boreholes$`_import_borehole_id_SQL`[i],
              ";"
            )
          )[1, 1]
          screen_bottom <- DBI::dbGetQuery(
            sql,
            paste0(
              "SELECT BottomOfScreen FROM WellRecords WHERE BoreholeID = ",
              boreholes$`_import_borehole_id_SQL`[i],
              ";"
            )
          )[1, 1]
          screen_comment <- retry(DBI::dbGetQuery(
            acc,
            paste0(
              "SELECT ScreenComm FROM 2_WELL WHERE BoreholeID = ",
              boreholes$`_import_borehole_id_SQL`[i],
              ";"
            )
          )[1, 1])
          static_wl <- DBI::dbGetQuery(
            sql,
            paste0(
              "SELECT StaticWaterLevel FROM WellRecords WHERE BoreholeID = ",
              boreholes$`_import_borehole_id_SQL`[i],
              ";"
            )
          )[1, 1]
          est_yield <- DBI::dbGetQuery(
            sql,
            paste0(
              "SELECT EstimatedYield FROM WellRecords WHERE BoreholeID = ",
              boreholes$`_import_borehole_id_SQL`[i],
              ";"
            )
          )[1, 1]
          comment <- paste0(
            if (!is.na(casing_comment)) casing_comment else NULL,
            if (!is.na(screen_comment)) screen_comment else NULL
          )

          df <- data.frame(
            borehole_id = new_borehole_id,
            casing_material = if (!is.na(casing_material_id[1])) {
              casing_material_id[1]
            } else {
              NA
            },
            casing_diameter_mm = if (!is.na(casing_diameter_mm)) {
              casing_diameter_mm
            } else if (!is.na(casing_diameter_in)) {
              casing_diameter_in * 25.4
            } else {
              NA
            }, # Convert inches to mm
            casing_depth_to_m = if (!is.na(casing_depth_to)) {
              casing_depth_to
            } else {
              NA
            },
            stick_up_height_m = if (!is.na(stick_up_height)) {
              stick_up_height * 0.3048
            } else {
              NA
            }, # Convert feet to meters
            notes = if (!nchar(comment) > 0) comment else NA,
            screen_top_depth_m = if (!is.na(screen_top)) {
              screen_top * 0.3048
            } else {
              NA
            }, # Convert feet to meters
            screen_bottom_depth_m = if (!is.na(screen_bottom)) {
              screen_bottom * 0.3048
            } else {
              NA
            }, # Convert feet to meters
            static_water_level_m = if (!is.na(static_wl)) {
              static_wl * 0.3048
            } else {
              NA
            }, # Convert feet to meters
            estimated_yield_lps = if (!is.na(est_yield)) {
              est_yield * 0.0630902
            } else {
              NA
      ...
      locations <- DBI::dbGetQuery(
        con,
        "SELECT location_id, name, latitude, longitude FROM locations WHERE latitude IS NOT NULL AND longitude IS NOT NULL AND location LIKE 'YOWN%';"
      )
      borehole_locs <- DBI::dbGetQuery(
        con,
        "SELECT borehole_id, borehole_name, latitude, longitude FROM boreholes.boreholes;"
      )
      ...
      for (i in 1:nrow(borehole_locs)) {
        ...
        dists <- geosphere::distHaversine(
          matrix(
            c(borehole_locs$longitude[i], borehole_locs$latitude[i]),
            ncol = 2
          ),
        ...
        if (min_dist <= 50) {
          # If the minimum distance is less than or equal to 50m, associate the borehole with that location
          ...
          response <- readline(
            prompt = paste0(
              ...
              "m from location '",
              location_name,
              "'. Associate with this location? (y/n): "
            )
          )
          if (tolower(response) == "y") {
            DBI::dbExecute(
              con,
              paste0(
                "UPDATE boreholes.boreholes SET location_id = ",
                loc_id,
                " WHERE borehole_id = ",
                borehole_locs$borehole_id[i],
                ";"
              )
            )
            message(
    ...
    DBI::dbExecute(
      con,
      "GRANT SELECT ON ALL TABLES IN SCHEMA boreholes TO PUBLIC;"
    )

    # Update the version_info table
    DBI::dbExecute(
      con,
      "UPDATE information.version_info SET version = '23' WHERE item = 'Last patch number';"
    )
    DBI::dbExecute(
      con,
      paste0(
  ...
  error = function(e) {
    ...
    stop(
      "Patch 23 failed and the DB has been rolled back to its earlier state. ",
      e$message
    )
  }
)
