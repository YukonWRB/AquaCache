...
message(
  "Working on Patch 21. Changes are being made within a transaction, so if something goes wrong, the database will be rolled back to its previous state (but you have a backup, right?)."
)

message(
  "This patch updates the timeseries_metadata views to give a more complete picture of the timeseries table and associated data."
)
...
tryCatch(
  ...
  '
    ...
    DBI::dbExecute(
      con,
      "CREATE OR REPLACE FUNCTION continuous.trunc_hour_utc(ts timestamptz)
RETURNS timestamptz
...
$$ SELECT date_trunc('hour', ts AT TIME ZONE 'UTC') AT TIME ZONE 'UTC'; $$;"
    ...
    DBI::dbExecute(
      con,
      "ALTER FUNCTION continuous.trunc_hour_utc(timestamptz) OWNER TO admin;"
    )

    # Improve performance of measurements_corrected_hourly view by dropping an ORDER BY clause
    DBI::dbExecute(
      con,
      "CREATE OR REPLACE VIEW continuous.measurements_hourly_corrected
...
"
    ...
    DBI::dbExecute(
      con,
      "CREATE INDEX measurements_continuous_hour_idx
    ...
    DBI::dbExecute(
      con,
      "UPDATE information.version_info SET version = '21' WHERE item = 'Last patch number';"
    )
    DBI::dbExecute(
      con,
      paste0(
  ...
  error = function(e) {
    ...
    stop(
      "Patch 21 failed and the DB has been rolled back to its earlier state. ",
      e$message
    )
  }
)
