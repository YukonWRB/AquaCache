...
message("Working on Patch 12. Changes are being made within a transaction, so if something goes wrong, the database will be rolled back to its previous state (but you have a backup, right?).")
...
tryCatch({
  ...
  message("Modifying the measurements_continuous view table so that deleted data is not shown.")
  DBI::dbExecute(con, "
  ...
  attr(con, "active_transaction") <<- FALSE
  stop("Patch 12 failed and the DB has been rolled back to its earlier state. ", e$message)
})
