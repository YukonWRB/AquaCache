...
message("Working on Patch 10. Changes are being made within a transaction, so if something goes wrong, the database will be rolled back to its previous state (but you have a backup, right?).")
...
tryCatch({
  ...
  DBI::dbExecute(con, "UPDATE pg_extension SET extrelocatable = TRUE WHERE extname = 'postgis';")
  ...
  raster_installed <- DBI::dbGetQuery(con, "SELECT COUNT(*) FROM pg_extension WHERE extname = 'postgis_raster';")
  if (raster_installed[1, 1] > 0) {
    DBI::dbExecute(con, "UPDATE pg_extension SET extrelocatable = TRUE WHERE extname = 'postgis_raster';")
  ...
  DBI::dbExecute(con, "UPDATE pg_extension SET extrelocatable = FALSE WHERE extname = 'postgis';")
  ...
  message("Patch 10 applied successfully: fixed postgis relocation to spatial schema.")
