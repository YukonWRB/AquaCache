...
# 2. Calculation versions can be kept - is that necessary, given the audit tables?
# 3. Some tables don't appear to have appropriate unique constraints yet, e.g.
#    discrete.result_aggregations
# 4. Update YGwater application pieces (when this is totally finalized). Consumption-only
#    modules/functions only require a change to 'plotDiscrete.R', but the 'admin'
#    side of the application (add/edit samples/results) will require updates. That
#    said, those modules are very much works in progress.
