...
message("Working on Patch 19. Changes are being made within a transaction, so if something goes wrong, the database will be rolled back to its previous state (but you have a backup, right?).")
...
tryCatch({
  ...
  DBI::dbExecute(con, "UPDATE information.version_info SET version = '19' WHERE item = 'Last patch number';")
  ...
  attr(con, "active_transaction") <<- FALSE
  stop("Patch 19 failed and the DB has been rolled back to its earlier state. ", e$message)
})
