...
tryCatch(
  {
    ...
    DBI::dbExecute(
      con,
      "ALTER TABLE owners_contributors ADD COLUMN name_fr TEXT;"
    )
  ...
  qualifier_type_description_fr TEXT
    ...
    qualifiers <- data.frame(
      ...
      qualifier_type_description = c(
        "Ice present",
        "Ice interpollation/estimation",
        "Dry well/stream/lake (not only sensor out of water)",
        "Sensor out of water",
        "Suspect measurements",
        "Estimated",
        "Draw-down after pumping",
        "Backwater affecting measurements",
        "Interpolated data",
        "High water missed (peak not recorded)",
        "Low water missed (trough not recorded)",
        "Peak montly maximum",
        "Peak monthly minimum",
        "Peak yearly maximum",
        "Peak yearly minimum",
        "Release of water, ex. beaver dam breaking",
        "Unspecified",
        "Unknown"
      ),
      qualifier_type_description_fr = c(
        "Glace présente",
        "Interpolation/estimation due à la glace",
        "Asséché (non seulement capteur hors de l'eau)",
        "Capteur hors de l'eau",
        "Mesures suspectes",
        "Estimé",
        "Abaissement après pompage",
        "Refoulement affectant les mesures",
        "Données interpolées",
        "Hautes eaux manquées (creux non enregistré)",
        "Basses eaux manquées (pic non enregistré)",
        "Pic mensuel maximum",
        "Pic mensuel minimum",
        "Pic annuel maximum",
        "Pic annuel minimum",
        "Libération d'eau, ex. rupture d'un barrage de castor",
        "Non spécifié",
        "Inconnu"
      )
    )
    ...
    if (!"qualifier_types" %in% tbls) {
      stop(
        "Table 'qualifier_types' was not created. Please check the database and try again."
      )
    } else {
      ...
      if (
        ...
        stop(
          "Table 'qualifier_types' was created but does not contain the expected columns. Please check the database and try again."
        )
      } else {
        codes <- DBI::dbGetQuery(
          con,
          "SELECT qualifier_type_code FROM qualifier_types;"
        )
        if (
          !all(
            c("ICE", "DRY", "SUS", "EST", "DD", "BW", "UNS", "UNK") %in%
              codes$qualifier_type_code
          )
        ) {
          stop(
            "Table 'qualifier_types' was created but does not contain the expected entries in qualifier_type_code. Please check the database and try again."
          )
        }
      }
    }
...
"
    ...
    DBI::dbExecute(
      con,
      "UPDATE approvals SET description = 'Unspecified', description_fr = 'Non spécifié' WHERE description = 'Undefined';"
    )
    DBI::dbExecute(
      con,
      "UPDATE grades SET description = 'Unspecified', description_fr = 'Non spécifié' WHERE description = 'Undefined';"
    )
    ...
    names(exist_table_approvals) <- c(
      "approval_type_code",
      "approval_type_description",
      "approval_type_description_fr"
    )
    exist_table_approvals[
      exist_table_approvals$approval_type_code == "Z",
      "approval_type_code"
    ] <- "UNK"
    exist_table_approvals[
      exist_table_approvals$approval_type_code == "U",
      "approval_type_code"
    ] <- "UNS"
    ...
    names(exist_table_grades) <- c(
      "grade_type_code",
      "grade_type_description",
      "grade_type_description_fr"
    )

    exist_table_grades <- exist_table_grades[
      exist_table_grades$grade_type_code %in%
        c("A", "B", "C", "D", "N", "Z", "U"),
    ]
    exist_table_grades[
      exist_table_grades$grade_type_code == "Z",
      "grade_type_code"
    ] <- "UNK"
    exist_table_grades[
      exist_table_grades$grade_type_code == "U",
      "grade_type_code"
    ] <- "UNS"
    exist_table <- rbind(
      exist_table_grades,
      data.frame(
        grade_type_code = c("E", "MISS"),
        grade_type_description = c("Estimated", "Missing data"),
        grade_type_description_fr = c("Estimé", "Données manquantes")
      )
    )
    ...
    DBI::dbExecute(
      con,
      "ALTER TABLE measurements_continuous DROP CONSTRAINT IF EXISTS fk_approval;"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE measurements_calculated_daily DROP CONSTRAINT IF EXISTS fk_approval;"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE rating_curves DROP CONSTRAINT IF EXISTS rating_curves_approval_fkey;"
    )
    ...
    DBI::dbExecute(
      con,
      "DROP FUNCTION IF EXISTS check_approval_exists_daily CASCADE;"
    )
    DBI::dbExecute(
      con,
      "DROP FUNCTION IF EXISTS check_approval_exists_continuous CASCADE;"
    )
    ...
    tsids <- DBI::dbGetQuery(
      con,
      "SELECT DISTINCT timeseries_id FROM measurements_continuous;"
    )[, 1]
    for (i in tsids) {
      exist <- DBI::dbGetQuery(
        con,
        paste0(
          "SELECT datetime, approval FROM measurements_continuous WHERE timeseries_id = ",
          i,
          " ORDER BY datetime;"
        )
      )
      earliest_daily <- DBI::dbGetQuery(
        con,
        paste0(
          "SELECT date AS datetime, approval FROM measurements_calculated_daily WHERE timeseries_id = ",
          i,
          " AND date = (SELECT MIN(date) FROM measurements_calculated_daily WHERE timeseries_id = ",
          i,
      ...
      exist <- rbind(
        exist,
        earliest_daily[earliest_daily$datetime < min(exist$datetime), ]
      )
    ...
    DBI::dbExecute(
      con,
      "ALTER TABLE measurements_continuous DROP COLUMN approval CASCADE;"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE measurements_calculated_daily DROP COLUMN approval CASCADE;"
    )
    ...
    if (nrow(exist_table_rc) > 0) {
      if (length(unique(exist_table_rc$approval)) > 0) {
        for (i in unique(exist_table_rc$approval)) {
          new_id <- approval_types[
            approval_types$approval_type_code == i,
            "approval_type_id"
          ]
          DBI::dbExecute(
            con,
            paste0(
              "UPDATE rating_curves SET approval = ",
              new_id,
              " WHERE approval = '",
              i,
              "';"
    ...
    DBI::dbExecute(
      con,
      "ALTER TABLE rating_curves ALTER COLUMN approval TYPE INTEGER USING approval::INTEGER;"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE rating_curves ADD CONSTRAINT rating_curves_approval_fkey FOREIGN KEY (approval) REFERENCES approval_types(approval_type_id) ON DELETE CASCADE ON UPDATE CASCADE;"
    )
    ...
    DBI::dbExecute(
      con,
      "ALTER TABLE measurements_continuous DROP CONSTRAINT IF EXISTS fk_grade;"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE measurements_calculated_daily DROP CONSTRAINT IF EXISTS fk_grade;"
    )
    DBI::dbExecute(
      con,
      "DROP FUNCTION IF EXISTS check_grade_exists_daily CASCADE;"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE locations_metadata_xsections DROP CONSTRAINT IF EXISTS locations_metadata_xsections_measurement_grade_fkey;"
    )
    DBI::dbExecute(
      con,
      "DROP FUNCTION IF EXISTS check_grade_exists_daily CASCADE;"
    ...
    DBI::dbExecute(
      con,
      "DROP FUNCTION IF EXISTS check_grade_exists_continuous CASCADE;"
    )
    ...
    DBI::dbExecute(
      con,
      "ALTER TABLE locations_metadata_xsections ALTER COLUMN measurement_grade TYPE INTEGER USING measurement_grade::INTEGER;"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE locations_metadata_xsections ADD CONSTRAINT locations_metadata_xsections_grade_fkey FOREIGN KEY (measurement_grade) REFERENCES grade_types(grade_type_id) ON DELETE CASCADE ON UPDATE CASCADE;"
    )
    ...
    for (i in tsids) {
      exist <- DBI::dbGetQuery(
        con,
        paste0(
          "SELECT datetime, grade FROM measurements_continuous WHERE timeseries_id = ",
          i,
          " ORDER BY datetime;"
        )
      )
      exist_daily <- DBI::dbGetQuery(
        con,
        paste0(
          "SELECT date AS datetime, grade FROM measurements_calculated_daily WHERE timeseries_id = ",
          i,
          " AND date < '",
          min(exist$datetime),
          "' AND value IS NOT NULL;"
      ...
      qualifiers_to_port[
        qualifiers_to_port$qualifier == "E",
        "qualifier"
      ] <- "EST"
      qualifiers_to_port[
        qualifiers_to_port$qualifier == "I",
        "qualifier"
      ] <- "ICE"
      qualifiers_to_port[
        qualifiers_to_port$qualifier == "R",
        "qualifier"
      ] <- "DD"
      qualifiers_to_port[
        qualifiers_to_port$qualifier == "S",
        "qualifier"
      ] <- "SUS"
      ...
      if (nrow(qualifiers_to_port) > 0) {
        adjust_qualifier(
          con,
          i,
          qualifiers_to_port[, c("datetime", "qualifier")]
        )
      }

      source_fx <- DBI::dbGetQuery(
        con,
        paste0(
          "SELECT source_fx FROM timeseries WHERE timeseries_id = ",
          i,
          ";"
        )
      )[1, 1]
    ...
    DBI::dbExecute(
      con,
      "ALTER TABLE measurements_continuous DROP COLUMN grade CASCADE;"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE measurements_calculated_daily DROP COLUMN grade CASCADE;"
    )
    ...
    DBI::dbExecute(
      con,
      "ALTER TABLE measurements_continuous DROP CONSTRAINT IF EXISTS measurements_continuous_owner_fkey;"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE measurements_calculated_daily DROP CONSTRAINT IF EXISTS calculated_daily_owner_fkey;"
    )
    ...
    wsc_owner <- DBI::dbGetQuery(
      con,
      "SELECT organization_id FROM owners_contributors WHERE name = 'Water Survey of Canada';"
    )[1, 1]
    if (is.na(wsc_owner)) {
      DBI::dbAppendTable(
        con,
        "owners_contributors",
        data.frame(name = "Water Survey of Canada")
      )
      wsc_owner <- DBI::dbGetQuery(
        con,
        "SELECT organization_id FROM owners_contributors WHERE name = 'Water Survey of Canada';"
    ...
    wrb_owner <- DBI::dbGetQuery(
      con,
      "SELECT organization_id FROM owners_contributors WHERE name = 'Yukon Department of Environment, Water Resources Branch';"
    )[1, 1]
    if (is.na(wrb_owner)) {
      DBI::dbAppendTable(
        con,
        "owners_contributors",
        data.frame(
          name = "Yukon Department of Environment, Water Resources Branch"
        )
      )
      wrb_owner <- DBI::dbGetQuery(
        con,
    ...
    eccc_owner <- DBI::dbGetQuery(
      con,
      "SELECT organization_id FROM owners_contributors WHERE name = 'Environment and Climate Change Canada';"
    )[1, 1]
    if (is.na(eccc_owner)) {
      DBI::dbAppendTable(
        con,
        "owners_contributors",
        data.frame(name = "Environment and Climate Change Canada")
      )
      eccc_owner <- DBI::dbGetQuery(
        con,
        "SELECT organization_id FROM owners_contributors WHERE name = 'Environment and Climate Change Canada';"
    ...
    usgs_owner <- DBI::dbGetQuery(
      con,
      "SELECT organization_id FROM owners_contributors WHERE name = 'United States Geological Survey';"
    )[1, 1]
    if (is.na(usgs_owner)) {
      DBI::dbAppendTable(
        con,
        "owners_contributors",
        data.frame(name = "United States Geological Survey")
      )
      usgs_owner <- DBI::dbGetQuery(
        con,
        "SELECT organization_id FROM owners_contributors WHERE name = 'United States Geological Survey';"
    ...
    for (i in tsids) {
      exist <- DBI::dbGetQuery(
        con,
        paste0(
          "SELECT datetime, owner FROM measurements_continuous WHERE timeseries_id = ",
          i,
          " ORDER BY datetime;"
        )
      )
      earliest_daily <- DBI::dbGetQuery(
        con,
        paste0(
          "SELECT date AS datetime, owner FROM measurements_calculated_daily WHERE timeseries_id = ",
          i,
          " AND date < '",
          min(exist$datetime),
          "' AND value IS NOT NULL;"
      ...
      owner <- DBI::dbGetQuery(
        con,
        paste0("SELECT owner FROM timeseries WHERE timeseries_id = ", i, ";")
      )[1, 1]
      if (is.na(source_fx)) {
        next
      }
    ...
    DBI::dbExecute(
      con,
      "ALTER TABLE measurements_continuous DROP COLUMN owner CASCADE;"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE measurements_calculated_daily DROP COLUMN owner CASCADE;"
    )
    ...
    DBI::dbExecute(
      con,
      "ALTER TABLE measurements_continuous DROP CONSTRAINT IF EXISTS measurements_continuous_contributor_fkey;"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE measurements_calculated_daily DROP CONSTRAINT IF EXISTS calculated_daily_contributor_fkey;"
    )
    ...
    for (i in tsids) {
      exist <- DBI::dbGetQuery(
        con,
        paste0(
          "SELECT datetime, contributor FROM measurements_continuous WHERE timeseries_id = ",
          i,
          " ORDER BY datetime;"
        )
      )
      earliest_daily <- DBI::dbGetQuery(
        con,
        paste0(
          "SELECT date AS datetime, contributor FROM measurements_calculated_daily WHERE timeseries_id = ",
          i,
          " AND date < '",
          min(exist$datetime),
          "' AND value IS NOT NULL;"
    ...
    DBI::dbExecute(
      con,
      "ALTER TABLE measurements_continuous DROP COLUMN contributor CASCADE;"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE measurements_calculated_daily DROP COLUMN contributor CASCADE;"
    )
...
"
    ...
    DBI::dbExecute(
      con,
      "COMMENT ON COLUMN images.img_meta_id IS 'Keyed to table images_index to associate an image with other images taken at this location. May be NULL for images not part of a series.';"
    )
    DBI::dbExecute(
      con,
      "COMMENT ON COLUMN images.latitude IS 'If the image is not part of an image series (and doesnt have an entry in column img_meta_id) this column must be populated. Enforced via trigger+function.';"
    )
    DBI::dbExecute(
      con,
      "COMMENT ON COLUMN images.longitude IS 'If the image is not part of an image series (and doesnt have an entry in column img_meta_id) this column must be populated. Enforced via trigger+function.';"
    )
    DBI::dbExecute(
      con,
      "COMMENT ON COLUMN images.azimuth_true IS 'Direction of the camera in degrees from true north.';"
    )
    DBI::dbExecute(
      con,
      "COMMENT ON COLUMN images.altitude_agl_m IS 'Altitude of the camera above ground level, in meters';"
    )
    DBI::dbExecute(
      con,
      "COMMENT ON COLUMN images.altitude_asl_m IS 'Altitude of the camera above sea level, in meters';"
    )

    # Add a column for 'owner' in the 'locations' table, referencing table owners_contributors
    DBI::dbExecute(
      con,
      "ALTER TABLE locations ADD COLUMN owner INTEGER REFERENCES owners_contributors(organization_id) ON DELETE SET NULL ON UPDATE CASCADE;"
    )

    # Update the version_info table
    DBI::dbExecute(
      con,
      "UPDATE information.version_info SET version = '3' WHERE item = 'Last patch number';"
    )
    DBI::dbExecute(
      con,
      paste0(
    ...
    message(
      "Patch 3 applied successfully: qualifiers, grades, approvals, owners, and contributors are now stored in their own tables. A full VACUUM ANALYZE is recommended: from R, use function vacuum() from this package."
    )
  ...
  error = function(e) {
    ...
    attr(con, "active_transaction") <<- FALSE
    stop(
      "Patch 3 failed and the DB has been rolled back to its earlier state. ",
      e$message
    )
  }
)
