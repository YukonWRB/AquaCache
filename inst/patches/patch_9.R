...
message(
  "Working on Patch 9. Changes are being made within a transaction, so if something goes wrong, the database will be rolled back to its previous state (but you have a backup, right?)."
)
...
tryCatch(
  {
    ...
    if (
      nrow(DBI::dbGetQuery(
        con,
        "SELECT 1 FROM pg_roles WHERE rolname = 'yg'"
      )) ==
        1
    ) {
    ...
    for (schema in schemas) {
      ...
      DBI::dbExecute(
        con,
        paste0("GRANT SELECT ON ALL TABLES IN SCHEMA ", schema, " TO yg;")
      )
      DBI::dbExecute(
        con,
        paste0("GRANT SELECT ON ALL SEQUENCES IN SCHEMA ", schema, " TO yg;")
      )
      DBI::dbExecute(
        con,
        paste0("GRANT EXECUTE ON ALL FUNCTIONS IN SCHEMA ", schema, " TO yg;")
      )
      DBI::dbExecute(
        con,
        paste0(
      ...
      DBI::dbExecute(
        ...
        paste0(
          "ALTER DEFAULT PRIVILEGES IN SCHEMA ",
          schema,
          " GRANT SELECT ON SEQUENCES TO yg;"
        )
      )
      DBI::dbExecute(
        con,
        paste0(
          "ALTER DEFAULT PRIVILEGES IN SCHEMA ",
          schema,
          " GRANT EXECUTE ON FUNCTIONS TO yg;"
        )
      )
    }
...
"
    ...
    DBI::dbExecute(
      con,
      "ALTER TABLE measurements_continuous DROP COLUMN share_with CASCADE;"
    ) # Should cascade to the RLS policy
    DBI::dbExecute(
      con,
      "DROP TRIGGER IF EXISTS validate_share_with_trigger_measurements_continuous ON measurements_continuous;"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE measurements_calculated_daily DROP COLUMN share_with CASCADE;"
    ) # Should cascade to the RLS policy
    DBI::dbExecute(
      con,
      "DROP TRIGGER IF EXISTS validate_share_with_trigger_measurements_calculated_daily ON measurements_calculated_daily;"
    )
    DBI::dbExecute(
      con,
      "DROP TRIGGER IF EXISTS validate_share_with_trigger_calculated_daily ON measurements_calculated_daily;"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE measurements_continuous DISABLE ROW LEVEL SECURITY;"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE measurements_continuous NO FORCE ROW LEVEL SECURITY;"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE measurements_calculated_daily DISABLE ROW LEVEL SECURITY;"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE measurements_calculated_daily NO FORCE ROW LEVEL SECURITY;"
    )
...
"
    ...
    for (i in tbl_names) {
      DBI::dbExecute(
        con,
        paste0("DROP TRIGGER IF EXISTS validate_share_with_trigger ON ", i, ";")
      )
      DBI::dbExecute(
        con,
        paste0(
          "DROP TRIGGER IF EXISTS validate_share_with_trigger_",
          i,
          " ON ",
          i,
          ";"
      ...
      DBI::dbExecute(
        con,
        paste0("DROP INDEX IF EXISTS ", i, "_share_with_gin_idx;")
      )
      policy_name <- DBI::dbGetQuery(
        con,
        paste0(
      ...
      DBI::dbExecute(
        ...
        paste0(
          "CREATE INDEX ",
          i,
          "_share_with_gin_idx ON ",
          i,
          " USING GIN (share_with);"
        )
      )
      DBI::dbExecute(
        con,
        paste0(
          "CREATE POLICY rls ON ",
          i,
          "
                              FOR ALL
                              USING (
    ...
    for (i in tbl_names) {
      DBI::dbExecute(
        ...
        paste0(
          "ALTER TABLE ",
          i,
          " ALTER COLUMN share_with SET DEFAULT '{public_reader}';"
        )
      )
    }
...
EXECUTE FUNCTION public.validate_share_with();"
    ...
    DBI::dbExecute(
      con,
      "CREATE INDEX idx_corrections_timeseries_date_range
...
"
    ...
    DBI::dbExecute(
      con,
      "CREATE OR REPLACE VIEW public.measurements_continuous_corrected AS
...
;"
    ...
    DBI::dbExecute(
      con,
      "CREATE OR REPLACE VIEW public.measurements_hourly_corrected AS
SELECT 
    mcc.timeseries_id,
...
ORDER BY 
    ...
    DBI::dbExecute(
      con,
      "CREATE OR REPLACE VIEW public.measurements_calculated_daily_corrected AS
SELECT 
    mcd.timeseries_id,
    mcd.date,
    mcd.value,
...
;"
    ...
    DBI::dbExecute(
      con,
      "REVOKE ALL ON measurements_continuous FROM public_reader;"
    )
    DBI::dbExecute(
      con,
      "REVOKE ALL ON measurements_calculated_daily FROM public_reader;"
    )
    ...
    DBI::dbExecute(
      con,
      "CREATE OR REPLACE FUNCTION public.enforce_share_with_restriction()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
              BEGIN
...
;"
    ...
    DBI::dbExecute(
      con,
      "CREATE OR REPLACE FUNCTION get_roles_with_select_on_locations()
...
"
    ...
    DBI::dbExecute(
      con,
      "GRANT EXECUTE ON FUNCTION get_roles_with_select_on_locations() TO public;"
    )

    # Update the version_info table
    DBI::dbExecute(
      con,
      "UPDATE information.version_info SET version = '9' WHERE item = 'Last patch number';"
    )
    DBI::dbExecute(
      con,
      paste0(
  ...
  error = function(e) {
    ...
    attr(con, "active_transaction") <<- FALSE
    stop(
      "Patch 9 failed and the DB has been rolled back to its earlier state. ",
      e$message
    )
  }
)
