...
DBI::dbExecute(
  con,
  'ALTER DATABASE "AquaCache" SET search_path TO public, instruments, information;'
)
...
DBI::dbExecute(
  con,
  "CREATE TABLE IF NOT EXISTS information.version_info (
...
DBI::dbExecute(
  con,
  "CREATE TRIGGER update_version_info_created_modified
...
tbl <- data.frame(
  item = c("AquaCache R package used for last patch", "Last patch number"),
  version = c(as.character(packageVersion("AquaCache")), "1")
)
...
if (nrow(check) != 2) {
  stop(
    "The table 'version_info' was not created successfully or is not in the right schema."
  )
}

# Give user a message
message(
  "Patch 1 applied successfully: created new schema 'information' and table 'version_info'."
)
