...
tryCatch(
  {
    ...
    DBI::dbExecute(
      con,
      "DROP TABLE IF EXISTS instruments.instrument_deployment;"
    )
  ...
  "
    ...
    for (table in tables_to_update) {
      ...
      DBI::dbExecute(
        ...
        paste0(
          "
      DROP TRIGGER IF EXISTS update_",
          table,
          "_modify_datetime ON instruments.",
          table,
          ";"
        )
      )
    ...
    DBI::dbExecute(
      con,
      "UPDATE information.version_info SET version = 2 WHERE item = 'Last patch number';"
    )
    DBI::dbExecute(
      con,
      paste0(
    ...
    # Give user a message
    message(
      "Patch 2 applied successfully: dropped table instruments.instrument_deployment and fixed trigger functions for instrument-realted tables."
    )
  },
  error = function(e) {
    ...
    attr(con, "active_transaction") <<- FALSE
    stop(
      "Patch 2 failed and the DB has been rolled back to its earlier state. ",
      e$message
    )
  }
)
