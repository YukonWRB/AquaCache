...
tryCatch(
  {
    ...
    DBI::dbExecute(
      con,
      "ALTER TABLE sample_series ALTER COLUMN source_fx_args TYPE JSONB USING source_fx_args::JSONB;"
    )
    ...
    for (i in 1:nrow(sample_series)) {
      ...
      if (!is.na(old_loc)) {
        new_args <- data.frame(location = loc_code, old_loc = old_loc)
      } else {
      ...
      DBI::dbExecute(
        con,
        paste0(
          "UPDATE sample_series SET source_fx_args = '",
          new_args_json,
          "' WHERE sample_series_id = ",
          row$sample_series_id,
          ";"
        )
      )
    }
    ...
    DBI::dbExecute(
      con,
      "ALTER TABLE timeseries ALTER COLUMN source_fx_args TYPE JSONB USING source_fx_args::JSONB;"
    )
    for (i in 1:nrow(timeseries)) {
      ...
      if (is.na(record_rate)) {
        remote_param <- DBI::dbGetQuery(
          con,
          paste0(
            "SELECT remote_param_name FROM fetch_settings WHERE parameter_id = ",
            parameter,
            " AND source_fx = '",
            source_fx,
            "' AND period_type = '",
            period_type,
            "' AND record_rate IS NULL;"
          )
        )[1, 1]
      } else {
        remote_param <- DBI::dbGetQuery(
      ...
      new_args <- data.frame(location = loc_code, parameter = remote_param)
    ...
    # raster_series_index ##################
    # fetch the current table
    raster_series_index <- DBI::dbGetQuery(
      con,
      "SELECT * FROM raster_series_index;"
    )
    # Delete the contents of column 'source_fx_args'
    ...
    DBI::dbExecute(
      con,
      "ALTER TABLE raster_series_index ALTER COLUMN source_fx_args TYPE JSONB USING source_fx_args::JSONB;"
    )
    ...
    for (i in 1:nrow(raster_series_index)) {
      ...
      DBI::dbExecute(
        con,
        paste0(
          "UPDATE raster_series_index SET source_fx_args = '",
          new_args_json,
          "' WHERE raster_series_id = ",
          row$raster_series_id,
          ";"
        )
      )
    }
    ...
    DBI::dbExecute(
      con,
      "ALTER TABLE image_series ALTER COLUMN source_fx_args TYPE JSONB USING source_fx_args::JSONB;"
    )
    ...
    img_types <- data.frame(
      image_type = c(
        "Sampling event",
        "Ice observation flight",
        "Site audit",
        "Spill response",
        "Documentation",
        "Auto",
      ...
      description = c(
        "Regular sampling event (not audit)",
    ...
    DBI::dbExecute(
      con,
      "ALTER TABLE images ADD COLUMN image_type INTEGER REFERENCES files.image_types(image_type_id);"
    )
    DBI::dbExecute(
      con,
      "ALTER TABLE images ADD COLUMN create_datetime TIMESTAMP DEFAULT NOW();"
    )

    # Update the version_info table
    DBI::dbExecute(
      con,
      "UPDATE information.version_info SET version = '14' WHERE item = 'Last patch number';"
    )
    DBI::dbExecute(
      con,
      paste0(
  ...
  error = function(e) {
    ...
    attr(con, "active_transaction") <<- FALSE
    stop(
      "Patch 14 failed and the DB has been rolled back to its earlier state. ",
      e$message
    )
  }
)
