# Reference docs: https://panama.aquaticinformatics.net/AQUARIUS/Publish/v2/docs/reference.html

# Create a simple AQUARIUS Time-Series API client.
timeseriesClient <- methods::setRefClass(
  "timeseriesClient",
  fields = list(
    version = "character",
  ...
  methods = list(
    connect = function(hostname, username, password) {
      ...
      if (isLegacy) {
        legacyPublishUri <<- paste0(
          prefix,
          hostname,
          "/AQUARIUS/Publish/AquariusPublishRestService.svc"
        )
        legacyAcquisitionUri <<- paste0(
          prefix,
          hostname,
    ...
    getTimeSeriesUniqueId = function(timeSeriesIdentifier) {
      if (isLegacy | !grepl("@", timeSeriesIdentifier)) {
        ...
        r <- httr::GET(
          paste0(publishUri, "/GetTimeSeriesDescriptionList"),
          query = list(LocationIdentifier = location)
        )
        httr::stop_for_status(
          r,
          paste("retrieve time-series at location", location)
        )

        # Find the unique ID by matching the full identifier
        j <- jsonlite::fromJSON(httr::content(r, "text"))
        uniqueId <- j$TimeSeries$UniqueId[which(
          j$TimeSeries$Identifier == timeSeriesIdentifier
        )]

        if (length(uniqueId) <= 0) {
          # Throw on the brakes
          stop(
            "Can't find time-series '",
            timeSeriesIdentifier,
            "' in location '",
            location,
            "'."
    ...
    getLocationIdentifier = function(timeSeriesIdentifier) {
      if (!grepl("@", timeSeriesIdentifier)) {
        stop(
          timeSeriesIdentifier,
          " is not a <Parameter>.<Label>@<Location> time-series identifier"
        )
      }
    ...
    formatIso8601 = function(datetime) {
      ...
      if (substr(isoText, len, len) != "Z") {
        ...
        isoText <- paste0(
          substr(isoText, 1, len - 2),
          ":",
          substr(isoText, len - 1, len)
        )
      }
      isoText
    },
    getLocationData = function(locationIdentifier) {
      locationData <- jsonlite::fromJSON(httr::content(
        httr::stop_for_status(
          httr::GET(
            paste0(publishUri, "/GetLocationData"),
    ...
    getMetadataChangeTransactionList = function(
      ...
      metadataChangeList <- jsonlite::fromJSON(httr::content(
        httr::stop_for_status(
          httr::GET(
            paste0(.self$publishUri, "/GetMetadataChangeTransactionList"),
            query = q
          ),
    ...
    getTimeSeriesCorrectedData = function(
      ...
      if (isLegacy) {
        ...
        q <- list(
          TimeSeriesUniqueId = .self$getTimeSeriesUniqueId(
            timeSeriesIdentifier
          ),
          QueryFrom = queryFrom,
          QueryTo = queryTo,
      ...
      data <- jsonlite::fromJSON(httr::content(
        httr::stop_for_status(
          httr::GET(
            paste0(.self$publishUri, "/GetTimeSeriesCorrectedData"),
            query = q
          ),
