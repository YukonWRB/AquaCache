...
db_dump <- function(name = "aquacache", host = Sys.getenv("aquacacheHost"), port = Sys.getenv("aquacachePort"), 
                        username = Sys.getenv("aquacacheAdminUser"), password = Sys.getenv("aquacacheAdminPass"), 
                        outpath = "choose", pg_dump = NULL, pg_dumpall = NULL, data = FALSE) {
  ...
  if (!file.exists(pg_dumpall)) {
    stop("The specified pg_dumpall utility does not exist or could not be found.")
  }
