...
downloadEQWin <- function(
  ...
  # Get the data
  StnId <- DBI::dbGetQuery(
    EQcon,
    paste0("SELECT StnId FROM eqstns WHERE StnCode = '", location, "';")
  )[1, 1]
  ...
  SampleIds <- DBI::dbGetQuery(
    EQcon,
    paste0(
      "SELECT SampleId, CollectDateTime, SampleClass FROM eqsampls WHERE StnId = ",
      StnId,
      " AND CollectDateTime >= #",
      substr(as.character(start_datetime), 1, 19),
      "# AND CollectDateTime <= #",
      substr(as.character(end_datetime), 1, 19),
      "#;"
    )
  )
  if (nrow(SampleIds) > 0) {
    SampleIds$CollectDateTime <- lubridate::force_tz(
      SampleIds$CollectDateTime,
      "MST"
    )
    meas <- DBI::dbGetQuery(
      EQcon,
      paste0(
        "SELECT SampleId, Result FROM eqdetail WHERE SampleId IN (",
        paste(SampleIds$SampleId, collapse = ", "),
        ");"
      )
    )
    if (nrow(meas) > 0) {
