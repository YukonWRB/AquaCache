...
readSnowTemplate <- function(template) {
  ...
  #for (s in c(2,3,5)) {
      ...
      calculated <- openxlsx::read.xlsx(xlsxFile = template, sheet = s, rows = c(12,23,25), cols = c(2,3,7), colNames=TRUE)
      ...
      tryCatch({
        ...
        message(paste0("Unable to retrieve location id using the following snow course name given in template: ", survey[1,2], ". This location name does not exist in the database. Check for typos in name."))
      ...
      ## location, target_date, survey_date, sampler_name, method
      ...
      if (is.na(target_date) | length(target_date)==0) {
        message(paste0("Snow survey target date is missing for snow coarse '", survey[1,2], "' (", loc_id, ") and must be given."))
        message("FAILED: new snow course data for '", survey[1,2], "' (", loc_id, ") not imported")
        next
      }
      if (is.na(survey_date) | length(survey_date)==0) {
        message(paste0("Snow survey sampling date is missing for snow coarse '", survey[1,2], "' (", loc_id, ") and must be given."))
        message("FAILED: new snow course data for '", survey[1,2], "' (", loc_id, ") not imported")
        next
      }
      ...
      if (method %in% c("average", "bulk")) {
        if (is.na(swe) | length(swe)==0) {
          message(paste0("SWE is missing for snow coarse '", survey[1,2], "' (", loc_id, ") and must be given."))
          message("FAILED: new snow course data for '", survey[1,2], "' (", loc_id, ") not imported")
          next
        }
        if (is.na(depth) | length(depth)==0) {
          message(paste0("Snow depth is missing for snow coarse '", survey[1,2], "' (", loc_id, ") and must be given."))
          message("FAILED: new snow course data for '", survey[1,2], "' (", loc_id, ") not imported")
          next
        }
      } else if (method == "standard") {
        if (length(swe) < length(sample_datetime)) {
          message(paste0("SWE is missing for 1 or more samples of snow course '", survey[1,2], "' (", loc_id, ") and must be given."))
          message("FAILED: new snow course data for '", survey[1,2], "' (", loc_id, ") not imported")
          next
        }
        if (length(depth) < length(sample_datetime)) {
          message(paste0("Snow depth is missing for 1 or more samples of snow course '", survey[1,2], "' (", loc_id, ") and must be given."))
          message("FAILED: new snow course data for '", survey[1,2], "' (", loc_id, ") not imported")
          next
        }
      }
      ...
      tryCatch({
        ...
        if (method == "standard"){survey_id <- rep(surv_id, times=length(sample_datetime))
        } else if (method %in% c("average", "bulk")) {survey_id <- surv_id}
        ...
        for (m in maint_db$maintenance) {
          # check if maintenance x has been removed in template
