...
getNewHRDPA <- function(start_datetime, url = "https://dd.weather.gc.ca/model_hrdpa/2.5km/") {

  # Check if there already exists a temporary file with the required interval, location, start_datetime, and end_datetime.
  saved_files <- list.files(paste0(tempdir(), "/getNewHRDPA"))
  if (length(saved_files) == 0){
    ...
    if (nrow(ok) > 0){
      ...
      available <- readRDS(paste0(tempdir(), "/getNewHRDPA/", target_file$file))
      file_exists <- TRUE
    } else {
      file_exists <- FALSE
    }
  ...
  if (!file_exists){
    ...
    suppressWarnings(dir.create(paste0(tempdir(), "/getNewHRDPA")))
    name <- gsub(" ", "", Sys.time())
    name <- gsub("-", "", name)
    name <- substr(gsub(":", "", name), 1,12)
    saveRDS(available, paste0(tempdir(), "/getNewHRDPA/", name, ".rds"))
  }
