...
#' Any document can be associated with locations (points), lines, polygons, or any combination thereof. Please reference the table 'points_lines_polygons' to give the correct geom_id(s) for your desired geoms.
#'
...
#' @param geoms The geom_id(s) with which to associate the document (must be in the database table 'points_lines_polygons'). Leave NULL for a document with no spatial context.
...
insertHydrometDocument <- function(path, name, type, description, authors = NULL, publish_date = NULL, url = NULL, geoms = NULL, con = hydrometConnect()){
  ...
  document_type <- tolower(document_type)
  if (!(document_type %in% c('thesis', 'report', 'well log', 'conference paper', 'poster', 'journal article', 'map', 'graph', 'protocol', 'grading scheme', 'metadata', 'other'))) {
  ...
  if (!is.null(geoms)){
    ...
    exist_geoms <- DBI::dbGetQuery(con, paste0("SELECT geom_id, geom_type, name, description FROM points_lines_polygons WHERE geom_id IN (", paste(geoms, collapse = ", "), ")"))
    if (nrow(exist_geoms) == 0){
      stop("None of the geom_ids you specified for parameter geoms can be found in the table points_lines_polygons. Try again.")
    }
    if (nrow(exist_geoms != geoms)){
      warning("At least one of the geom_ids you specified for parameter geoms could not be found in table points_lines_polygons.")
    }
  }
