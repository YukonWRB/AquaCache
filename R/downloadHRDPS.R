...
downloadHRDPS <- function(
  ...
  check_url <- paste0(
    ...
    curr.date,
    "/WXO-DD/model_hrdps/continental/2.5km/"
  )
  ...
  if (length(links_now) == 0) {
    stop(
      "downloadHRDPS: Could not find any forecast directories on the remote server."
    )
  }
  ...
  latest_issue <- data.frame(
    links = paste0(check_url, links_now),
    issue = as.POSIXct(
      paste0(curr.date, substr(links_now, 1, 2), ":00:00"),
      format = "%Y%m%d%H:%M:%S",
      tz = "UTC"
    )
  )
  if (start_datetime >= max(latest_issue$issue)) {
    ...
    latest_issue <- latest_issue[
      latest_issue$issue == max(latest_issue$issue),
    ]
    ...
    if (length(files) == 0) {
      message(
        "downloadHRDPS: No new rasters were found for the specified parameter."
      )
      return(NULL)
    }
    files[["forecast"]] <- TRUE
