...
downloadHRDPS <- function(
    ...
    for (i in c("00", "06", "12", "18")) {
      ...
      for (j in 1:length(links)) {
        timestep_links <- rvest::session(paste0(url, i, "/", links[j])) |>
          rvest::html_elements("a") |>
          rvest::html_attr("href")
        timestep_links <- timestep_links[grep(
          "^[0-9]{8}T[0-9]{2}Z",
          timestep_links
        )]
        if (length(timestep_links) > 0) {
          tmp <- data.frame(
            link = paste0(url, i, "/", links[j], "/", timestep_links),
            issue = as.POSIXct(
              substr(timestep_links, 1, 11),
              format = "%Y%m%dT%H",
              tz = "UTC"
            ),
            valid = as.POSIXct(
  ...
  available <- if (
    nrow(available[available$issue == max(available$issue), ]) == 48
  ) {
    available[available$issue == max(available$issue), ]
  } else {
    available[available$issue == max(available$issue) - 60 * 60 * 6, ]
  }
