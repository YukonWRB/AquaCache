...
calculate_period <- function(data, timeseries_id, con = NULL) {
  ...
  if (length(smoothed_diffs) > 0) {
    for (j in 1:length(smoothed_diffs)) {
      if (!is.na(smoothed_diffs[j]) && smoothed_diffs[j] != last_diff) {
        # Check if smoothed interval is not the same as the last recorded diff
        consecutive_count <- consecutive_count + 1
        if (consecutive_count == 3) {
          # At three consecutive new measurements it's starting to look like a pattern
          last_diff <- smoothed_diffs[j]
          change <- data.frame(
            datetime = data$datetime[j - 2],
            period = last_diff
          )
  ...
  if (nrow(changes) > 0) {
    for (j in 1:nrow(changes)) {
      ...
      seconds <- round(
        ((remaining_hours - floor(remaining_hours)) * 60 - minutes) * 60
      )
      data[data$datetime == changes$datetime[j], "period"] <- paste(
        "P",
        days,
        "DT",
        floor(remaining_hours),
        "H",
        minutes,
        "M",
        seconds,
    ...
    if (nrow(changes) > 0) {
      for (k in 1:nrow(changes)) {
        ...
        data[data$datetime == changes$datetime[k], "period"] <- paste(
          "P",
          days,
          "DT",
          floor(remaining_hours),
