...
addACOrg <- function(
  ...
  if (
    !is.na(name_fr) & (!inherits(name_fr, "character") | length(name_fr) != 1)
  ) {
    stop("Parameter 'name_fr' must be a character vector of length 1.")
  }
  if (
    !is.na(contact_name) &
      (!inherits(contact_name, "character") | length(contact_name) != 1)
  ) {
    stop("Parameter 'contact_name' must be a character vector of length 1.")
  }
  if (
    !is.na(phone) &
      (!inherits(phone, "character") &
        !inherits(phone, "numeric") |
        length(phone) != 1)
  ) {
