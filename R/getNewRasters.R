...
getNewRasters <- function(raster_series_ids = "all", con = AquaCacheCon(silent = TRUE), keep_forecasts = 'selective', active = 'default') {
    ...
    tryCatch({
      ...
      if (!is.null(rasters)) {
        for (j in 1:length(rasters)) {
          ...
          if (!is.na(exists) & is.na(flag)) { #If the raster already exists and the new one is not a prelim, delete the prelim one and replace.
        ...
        if (forecast) { # Delete the old forecast rasters as per input parameters, adjust raster_series_index.last_issue
          valid_from <- as.POSIXct(sapply(rasters, function(x) x$valid_from), tz = "UTC")
          if (keep_forecasts == 'selective') {
            ...
            DBI::dbExecute(con, paste0("DELETE FROM rasters_reference WHERE raster_series_id = ", id, " AND valid_from IN ('", paste(valid_from, collapse = "', '"), "') AND issued NOT BETWEEN '", issued - 5, "' AND '", issued + 5, "';"))
          } else if (keep_forecasts == 'none') {
            # Delete all rasters from this series
            DBI::dbExecute(con, paste0("DELETE FROM rasters_reference WHERE raster_series_id = ", id, " AND issued NOT BETWEEN '", issued - 5, "' AND '", issued + 5, "';"))
            # Update start_datetime in the relevant table
            DBI::dbExecute(con, paste0("UPDATE raster_series_index SET last_issue = '", issued_datetime, "', start_datetime = '", min(valid_from), "' WHERE raster_series_id = ", id, ";"))
