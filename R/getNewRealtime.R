 ...
 getNewRealtime <- function(con = hydrometConnect(silent=TRUE), timeseries_id = "all")
{
  ...
  if (timeseries_id[1] == "all"){
    ...
    all_timeseries <- DBI::dbGetQuery(con, paste0("SELECT location, parameter, timeseries_id, source_fx, end_datetime, type FROM timeseries WHERE timeseries_id IN ('", paste(timeseries_id, collapse = "', '"), "') AND category = 'continuous'"))
  ...
  for (i in 1:nrow(all_timeseries)){
    ...
    tryCatch({
      ...
      warning("getNewRealtime: Failed to check for new data or to append discovered new data at location ", loc, " and parameter ", parameter, " (timeseries_id ", all_timeseries$timeseries_id[i], ").")
    }) #End of tryCatch
