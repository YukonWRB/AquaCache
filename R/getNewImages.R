...
getNewImages <- function(image_meta_ids = "all", con = hydrometConnect(silent = TRUE)) {
  ...
  if (image_meta_ids[1] == "all") {
    meta_ids <- DBI::dbGetQuery(con, "SELECT i.img_meta_id, i.last_img, i.source_fx, i.source_fx_args, l.location FROM images_index i JOIN locations l ON i.location_id = l.location_id WHERE i.img_type = 'auto' AND i.source_fx IS NOT NULL;")
  } else {
    meta_ids <- DBI::dbGetQuery(con, paste0("SELECT i.img_meta_id, i.last_img, i.source_fx, i.source_fx_args, l.location FROM images_index i JOIN locations l ON i.location_id = l.location_id WHERE i.img_type = 'auto' AND i.source_fx IS NOT NULL AND img_meta_id IN ('", paste(image_meta_ids, collapse = "', '"), "');"))
