...
getNewImages <- function(image_meta_ids = "all", con = hydrometConnect(silent = TRUE)) {
  ...
  if (image_meta_ids[1] == "all") {
    meta_ids <- DBI::dbGetQuery(con, "SELECT img_meta_id, geom_id, last_img, source_fx, source_fx_args FROM images_index WHERE img_type = 'auto' AND source_fx IS NOT NULL;")
  } else {
    meta_ids <- DBI::dbGetQuery(con, paste0("SELECT img_meta_id, geom_id, last_img, source_fx, source_fx_args FROM images_index WHERE img_meta_id IN ('", paste(image_meta_ids, collapse = "', '"), "') AND img_type = 'auto' AND source_fx IS NOT NULL;"))
  ...
  for (i in 1:nrow(meta_ids)) {
    ...
    location <- DBI::dbGetQuery(con, paste0("SELECT location FROM locations WHERE locations.geom_id = ", meta_ids[i, "geom_id"], ";"))[1,1]
