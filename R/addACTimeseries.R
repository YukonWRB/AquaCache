...
addACTimeseries <- function(df = NULL, data = NULL, start_datetime = NA, location = NA, z = NA, parameter = NA, media = NA, sensor_priority = 1,  period_type = 'instantaneous', record_rate = NA, share_with = "public_reader", owner = NA, source_fx = NA, source_fx_args = NA, note = NA, con = NULL) {
  ...
  if (!is.null(df)) {
    ...
    if (!all(is.na(c(location, start_datetime, z, parameter, media, record_rate, owner, source_fx, source_fx_args, note)))) {
  ...
  for (i in 1:length(location)) {
    ...
    if (nrow(setting) == 0) {
      stop("There is no existing entry in the 'fetch_settings' table for source_fx ", sfx, ", parameter ", param, ", record_rate = ", rec_rate, ", and period_type ", p_type, ". Please add a corresponding entry to the database table 'settings' to address this and try again. I need to know how to fetch the data for this timeseries!!!")
    }
  }
      ...
      if (!is.null(data)) {
        if (!is.na(data[i])) {
          if ('date' %in% colnames(data[[i]])) {
            ...
            addNewContinuous(tsid = new_tsid, df = data[[i]], target = 'realtime', con = con)
