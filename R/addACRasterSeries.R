...
#' @details Source functions and arguments are stored as ordered assignments.
#' The active assignment with the lowest `fetch_priority` is used by
#' [getNewRasters()]. Numeric vectors in a named-list `source_fx_args`, such as
#' a raster clipping bounding box, are serialized as JSON arrays and decoded
#' back to numeric vectors before the source function is called.
#'
...
#' @param source_adapters An assignment data frame with columns `source_fx`,
#'   optional JSON or named-list `source_fx_args`, `fetch_priority`, `active`,
#'   and optional `note`. Functions must be enabled for the raster domain.
...
#'
#' @return TRUE if successful, and a new entry in the database with rasters fetched.
#' @export
#'
#' @examples
#' \dontrun{
#' raster_sources <- data.frame(
#'   source_fx = "downloadCaLDAS",
#'   source_fx_args = I(list(list(
#'     parameter = "SnowWaterEquiv_Sfc",
#'     clip = c(70, -142, 59, -123),
#'     hrs = c(0, 6, 12, 18)
#'   ))),
#'   fetch_priority = 1L,
#'   active = TRUE,
#'   note = "Four CaLDAS analyses per day, clipped to a bounding box."
#' )
#'
#' addACRasterSeries(
#'   model = "CaLDAS",
...
#'   source_adapters = raster_sources,
#'   type = "reanalysis"
#' )
#' }
...
addACRasterSeries <- function(
  ...
  if (!nrow(source_adapters)) {
    stop("At least one raster source-adapter assignment is required.")
  }
  ...
  if (is.na(raster_type_id)) {
    stop(
      "The raster type you specified could not be found in spatial.raster_types."
    )
  }
  ...
  if (nrow(parameter_lookup) != 1) {
    stop(
      "The parameter you specified could not be resolved to exactly one row in public.parameters."
    )
  }
  ...
  if (!is.na(media_id)) {
    ...
    if (is.na(media_exists)) {
      stop(
        "The media_id you specified could not be found in public.media_types."
      )
    }
  }
  ...
  if (!is.na(aggregation_type)) {
    ...
    if (nrow(aggregation_lookup) != 1) {
      stop(
        "The aggregation_type you specified could not be resolved in continuous.aggregation_types."
      )
    }
  ...
  if (
    !is.na(z_units) && (!inherits(z_units, "character") || length(z_units) != 1)
  ) {
