...
#' @param parameter A WSC parameter code. 47 for discharge primary (sensor derived), 8 for discharge (sensor measured), 46 for level, 5 for water temperature, 4 for equipment temperature, 1 for air temperature. See the full list using [tidyhydat::param_id].
...
downloadWSC <- function(
  ...
  datetime_string <- paste0(
    "start_date=",
    substr(start_datetime, 1, 10),
    "%20",
    substr(start_datetime, 12, 19),
    "&end_date=",
    substr(end_datetime, 1, 10),
    "%20",
    substr(end_datetime, 12, 19)
  )
  url <- paste0(
    baseurl,
    location_string,
  ...
  data <- data.table::fread(
    ...
    select = c(
      ...
      "Grade/Classification" = "numeric",
      "Approval/Approbation" = "character",
      "Qualifiers/Qualificatifs" = "numeric"
    ),
    col.names = c("datetime", "value", "grade", "approval", "qualifier")
  )
  ...
  if (nrow(data) > 0) {
    ...
    approval_mapping <- c(
      ...
      "Approved/Approuv\u00E9e" = approvals_DB[
        approvals_DB$approval_type_code == "A",
        "approval_type_id"
      ],
      "Provisional/Provisoire" = approvals_DB[
        approvals_DB$approval_type_code == "N",
        "approval_type_id"
      ],
      "Preliminary/Pr\u00E9liminaire" = approvals_DB[
        approvals_DB$approval_type_code == "N",
        "approval_type_id"
      ],
      "Checked/Verifi\u00E9e" = approvals_DB[
        approvals_DB$approval_type_code == "R",
        "approval_type_id"
      ],
      "Unspecified/Non sp\u00E9cifi\u00E9" = approvals_DB[
        approvals_DB$approval_type_code == "UNS",
        "approval_type_id"
      ],
      "Undefined/Non d\u00E9fini" = approvals_DB[
        approvals_DB$approval_type_code == "UNS",
        "approval_type_id"
      ]
    )
