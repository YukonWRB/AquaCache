...
#' To store credentials in `.Renviron`, call [usethis::edit_r_environ()] and
#' enter `AQUSER`, `AQPASS`, and `AQSERVER`. Credentials may instead be stored
#' in a source assignment's `source_fx_args`, but are then unencrypted in the
#' database.
...
downloadAquarius <- function(
  ...
  if (nrow(ts) > 0) {
    ...
    offset <- substr(
      ts$datetime[1],
      nchar(ts$datetime[1]) - 5,
      nchar(ts$datetime[1])
    )
    offset <- gsub(":", "", offset)
    ts$datetime <- paste0(
      substr(ts$datetime, 1, nchar(ts$datetime) - 6),
      offset
    )
    ...
    if (is.null(nrow(RawDL$Approvals)) || nrow(RawDL$Approvals) == 0) {
      ...
      approvals <- data.frame(
        level = approvals_DB[
          approvals_DB$approval_type_code == "UNS",
          "approval_type_id"
        ],
        start_time = min(ts$datetime),
        end_time = max(ts$datetime)
      )
    } else {
      approvals <- RawDL$Approvals[, c("ApprovalLevel", "StartTime", "EndTime")]
      stoffset <- substr(
        approvals$StartTime[1],
        nchar(approvals$StartTime[1]) - 5,
        nchar(approvals$StartTime[1])
      )
      stoffset <- gsub(":", "", stoffset)
      approvals$StartTime <- paste0(
        substr(approvals$StartTime, 1, nchar(approvals$StartTime) - 6),
        stoffset
      )
      ...
      endoffset <- substr(
        approvals$EndTime[1],
        nchar(approvals$EndTime[1]) - 5,
        nchar(approvals$EndTime[1])
      )
      endoffset <- gsub(":", "", endoffset)
      approvals$EndTime <- paste0(
        substr(approvals$EndTime, 1, nchar(approvals$EndTime) - 6),
        endoffset
      )
      ...
      approval_mapping <- c(
        ...
        "900" = approvals_DB[
          approvals_DB$approval_type_code == "C",
          "approval_type_id"
        ],
        "950" = approvals_DB[
          approvals_DB$approval_type_code == "C",
      ...
      approvals$level <- ifelse(
        as.character(approvals$level) %in% names(approval_mapping),
    ...
    if (is.null(nrow(RawDL$Grades)) || nrow(RawDL$Grades) == 0) {
      ...
      grades <- data.frame(
        level = grades_DB[grades_DB$grade_type_code == "UNS", "grade_type_id"],
        start_time = min(ts$datetime),
        end_time = max(ts$datetime)
      )
    } else {
      ...
      stoffset <- substr(
        grades$StartTime[1],
        nchar(grades$StartTime[1]) - 5,
        nchar(grades$StartTime[1])
      )
      stoffset <- gsub(":", "", stoffset)
      grades$StartTime <- paste0(
        substr(grades$StartTime, 1, nchar(grades$StartTime) - 6),
        stoffset
      )
      ...
      endoffset <- substr(
        grades$EndTime[1],
        nchar(grades$EndTime[1]) - 5,
        nchar(grades$EndTime[1])
      )
      endoffset <- gsub(":", "", endoffset)
      grades$EndTime <- paste0(
        substr(grades$EndTime, 1, nchar(grades$EndTime) - 6),
        endoffset
      )
      ...
      grade_mapping <- c(
        "0" = grades_DB[grades_DB$grade_type_code == "UNS", "grade_type_id"],
        ...
        "2" = grades_DB[grades_DB$grade_type_code == "E", "grade_type_id"],
        "10" = grades_DB[grades_DB$grade_type_code == "HD", "grade_type_id"],
        "11" = grades_DB[grades_DB$grade_type_code == "HI", "grade_type_id"],
      ...
      grades$level <- ifelse(
        as.character(grades$level) %in% names(grade_mapping),
    ...
    if (is.null(nrow(RawDL$Qualifiers)) || nrow(RawDL$Qualifiers) == 0) {
      ...
      stoffset <- substr(
        qualifiers$StartTime[1],
        nchar(qualifiers$StartTime[1]) - 5,
        nchar(qualifiers$StartTime[1])
      )
      stoffset <- gsub(":", "", stoffset)
      qualifiers$StartTime <- paste0(
        substr(qualifiers$StartTime, 1, nchar(qualifiers$StartTime) - 6),
        stoffset
      )
      qualifiers$StartTime <- as.POSIXct(
        qualifiers$StartTime,
        format = "%Y-%m-%dT%H:%M:%OS%z",
        tz = "UTC"
      )
      endoffset <- substr(
        qualifiers$EndTime[1],
        nchar(qualifiers$EndTime[1]) - 5,
        nchar(qualifiers$EndTime[1])
      )
      endoffset <- gsub(":", "", endoffset)
      qualifiers$EndTime <- paste0(
        substr(qualifiers$EndTime, 1, nchar(qualifiers$EndTime) - 6),
        endoffset
      )
      qualifiers$EndTime <- as.POSIXct(
        qualifiers$EndTime,
        format = "%Y-%m-%dT%H:%M:%OS%z",
        tz = "UTC"
      )
      ...
      qualifier_mapping <- c(
        "BKW" = qualifiers_DB[
          qualifiers_DB$qualifier_type_code == "BW",
          "qualifier_type_id"
        ],
        "DD" = qualifiers_DB[
          qualifiers_DB$qualifier_type_code == "DD",
          "qualifier_type_id"
        ],
        "DRY" = qualifiers_DB[
          qualifiers_DB$qualifier_type_code == "DRY",
          "qualifier_type_id"
        ],
        "E" = qualifiers_DB[
        ...
        "ES" = qualifiers_DB[
          qualifiers_DB$qualifier_type_code == "SUS",
          "qualifier_type_id"
        ],
        "FI" = qualifiers_DB[
          qualifiers_DB$qualifier_type_code == "INT",
          "qualifier_type_id"
        ],
        "HW-MISS" = qualifiers_DB[
          qualifiers_DB$qualifier_type_code == "HW-MISS",
          "qualifier_type_id"
        ],
        "ICE" = qualifiers_DB[
        ...
        "ICE-EST" = qualifiers_DB[
          qualifiers_DB$qualifier_type_code == "ICE-EST",
          "qualifier_type_id"
        ],
        "LW-MISS" = qualifiers_DB[
          qualifiers_DB$qualifier_type_code == "LW-MISS",
          "qualifier_type_id"
        ],
        "OOW" = qualifiers_DB[
          qualifiers_DB$qualifier_type_code == "OOW",
          "qualifier_type_id"
        ],
        "PMMAX" = qualifiers_DB[
          qualifiers_DB$qualifier_type_code == "PMMAX",
          "qualifier_type_id"
        ],
        "PMMIN" = qualifiers_DB[
          qualifiers_DB$qualifier_type_code == "PMMIN",
          "qualifier_type_id"
        ],
        "PYMAX" = qualifiers_DB[
          qualifiers_DB$qualifier_type_code == "PYMAX",
          "qualifier_type_id"
        ],
        "PYMIN" = qualifiers_DB[
          qualifiers_DB$qualifier_type_code == "PYMIN",
          "qualifier_type_id"
        ],
        "REL" = qualifiers_DB[
          qualifiers_DB$qualifier_type_code == "REL",
          "qualifier_type_id"
        ]
      )
      qualifiers$level <- ifelse(
        as.character(qualifiers$level) %in% names(qualifier_mapping),
    ...
    apply_intervals <- function(
      ...
      # Loop through each interval and assign the qualifier/approval/grade
      for (i in seq_along(start_idx)) {
        st <- df[[time_col]][start_idx[i]]
        ed <- df[[time_col]][end_idx[i]]
        sel <- df[[time_col]] >= st & df[[time_col]] <= ed

        # append if already present for qualifiers, otherwise replace
        if (new_col == "qualifier") {
          ...
          df[[new_col]][sel] <- intervals$level[i]
        }
      }
      return(df)
    }
    ...
    ts$grade[is.na(ts$grade)] <- grades_DB[
      grades_DB$grade_type_code == "UNS",
      "grade_type_id"
    ]
    ts$approval[is.na(ts$approval)] <- approvals_DB[
      approvals_DB$approval_type_code == "UNS",
      "approval_type_id"
    ]
    ts$qualifier[is.na(ts$qualifier)] <- qualifiers_DB[
      qualifiers_DB$qualifier_type_code == "UNS",
      "qualifier_type_id"
    ]
