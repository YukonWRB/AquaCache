...
dailyUpdate <- function(con = NULL, timeseries_id = "all", sample_series_id = "all", active = 'default', continuous = TRUE, discrete = TRUE, hydat = TRUE, images = TRUE, rasters = TRUE)
{
  ...
  if (nrow(continuous_ts) > 0) {
    ...
    tryCatch({
      stat_start <- Sys.time()
      calc_ts <- DBI::dbGetQuery(con, paste0("SELECT timeseries_id, last_daily_calculation, last_new_data FROM timeseries WHERE timeseries_id IN ('", paste(continuous_ts$timeseries_id, collapse = "', '"), "') AND record_rate <= '1 day';"))
