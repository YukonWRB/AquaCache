...
dailyUpdate <- function(
  ...
  if (continuous) {
    if (nrow(continuous_ts) > 0) {
      message(
        "Getting continuous information up to date with getNewContinuous..."
      )
      tryCatch(
        {
          rt_start <- Sys.time()
          ...
          message(
            "getNewContinuous executed in ",
            round(rt_duration[[1]], 2),
            " ",
            units(rt_duration),
            "."
          )
        ...
        error = function(e) {
          warning(
            "dailyUpdate: error fetching new continuous data. Returned message: ",
            e$message
          )
        }
      )
      ...
      tryCatch(
        {
          ...
          getNewDiscrete(
            con = con,
            sample_series_id = discrete_ids$sample_series_id,
            location_id = NULL,
            sub_location_id = NULL
          )
          disc_duration <- Sys.time() - disc_start
          message(
            "getNewDiscrete executed in ",
            round(disc_duration[[1]], 2),
            " ",
            units(disc_duration),
            "."
          )
        ...
        error = function(e) {
          warning(
            "dailyUpdate: error fetching new discrete data. Returned message: ",
            e$message
          )
        }
      )
    ...
    tryCatch(
      {
        ...
        message(
          "getNewImages executed in ",
          round(img_duration[[1]], 2),
          " ",
          units(img_duration),
          "."
        )
      ...
      error = function(e) {
        warning(
          "dailyUpdate: error fetching new images. Returned message: ",
          e$message
        )
      }
    )
    ...
    tryCatch(
      {
        ...
        message(
          "getNewRasters executed in ",
          round(ras_duration[[1]], 2),
          " ",
          units(ras_duration),
          "."
        )
      ...
      error = function(e) {
        warning(
          "dailyUpdate: error fetching new rasters. Returned message: ",
          e$message
        )
      }
    )
    ...
    tryCatch(
      {
        ...
        if (new_hydat) {
          ...
          message(
            "A new version of HYDAT was detected. Timeseries were updated in ",
            round(hy_duration[[1]], 2),
            " ",
            units(hy_duration),
            "."
          )
  ...
  message(
    "Total elapsed time for dailyUpdate: ",
    round(total_diff[[1]], 2),
    " ",
    units(total_diff),
    ". End of function."
  )
