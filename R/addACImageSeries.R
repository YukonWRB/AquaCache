...
addACImageSeries <- function(location, start_datetime, source_fx, source_fx_args = NA, share_with = "public_reader", visibility_public = 'exact', con = NULL) {
  ...
  exists <- DBI::dbGetQuery(con, paste0("SELECT img_series_id FROM image_series WHERE location_id = ", location_id, ";"))[1,1]
  if (!is.na(exists)) {
  ...
  if (length(added) == 0) {
    ...
    first_new <- DBI::dbGetQuery(con, paste0("SELECT MIN(datetime) FROM images WHERE img_series_id = ", res, ";"))[1,1]
    DBI::dbExecute(con, paste0("UPDATE image_series SET first_img = '", first_new, "' WHERE img_series_id = ", res, ";"))
    message("Added new image series for location_id ", location_id, " and type 'auto'. The new img_series_id is ", res, ".")
  }
}
